...
.dtl-md{max-width:840px;width:95vw}
.dtl-hleft{display:flex;align-items:center;gap:10px;min-width:0}
.dtl-hicon{width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff}
.dtl-htitles{display:flex;flex-direction:column;gap:3px;min-width:0}
.dtl-htitles h2{font-size:0.72rem;color:#fff;font-weight:700;line-height:1.1}
.dtl-hrow2{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.dtl-hcode{font-size:0.82rem;color:#fff;font-weight:800;font-family:'SF Mono','Fira Code','Consolas',monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:280px}
.dtl-verif-pill{display:inline-flex;align-items:center;gap:6px;background:#0071c2;color:#fff;border-radius:20px;padding:3px 12px 3px 8px;font-size:0.62rem;font-weight:700;white-space:nowrap;flex-shrink:0;box-shadow:0 1px 4px rgba(0,0,0,.15)}
.dtl-body{padding:14px 18px 4px;display:flex;flex-direction:column;gap:10px}
.dtl-grid2{display:grid;grid-template-columns:1fr 1fr;gap:12px;align-items:stretch}
.dtl-col{display:flex;flex-direction:column;gap:7px}
...
.dtl-item.compact{padding:6px 12px}
...
.dtl-codes-card{background:var(--g1);border:1px solid var(--g2);border-radius:9px;padding:10px 11px;display:flex;flex-direction:column;gap:6px;height:100%}
.dtl-codes-title{display:flex;align-items:center;gap:8px;font-size:0.62rem;font-weight:700;color:var(--bm);text-transform:uppercase;letter-spacing:.6px;margin-bottom:1px}
.dtl-codes-ic{width:24px;height:24px;border-radius:7px;background:var(--bl);color:var(--bm);display:flex;align-items:center;justify-content:center;flex-shrink:0}
.dtl-code-row{display:flex;align-items:center;gap:9px;background:linear-gradient(135deg,var(--bl),#dcecfa);border:1px solid #BEDCF5;border-radius:7px;padding:6px 10px}
.dtl-code-num{width:19px;height:19px;border-radius:50%;background:var(--bm);color:#fff;font-size:0.58rem;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.dtl-code-val{flex:1;font-family:'SF Mono','Fira Code','Consolas',monospace;font-weight:800;color:var(--bd);font-size:0.85rem;word-break:break-all;letter-spacing:.01em}
.dtl-code-copy{background:none;border:none;color:var(--bm);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:4px;border-radius:5px;flex-shrink:0}
.dtl-code-copy:hover{background:#fff}
@media(max-width:680px){.dtl-grid2{grid-template-columns:1fr}.dtl-hcode{max-width:180px}}
...
const VerifBadge = ({ count, threshold, yaVerifique, loading, onClick, size = 26 }) => {
  ...
  return (
    ...
    >
      ...
      {/* palomita 1 */}
      <path d="M4.5 13.2l2.6 2.6 3.4-5.2" fill="none" stroke={check1?bright:dim} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      {/* palomita 2 */}
      <path d="M11.5 13.2l2.6 2.6 6.4-8.4" fill="none" stroke={check2?bright:dim} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
};
...
const ModalDetail = ({ record, onClose, onEdit, onReported, onVerified }) => {
  ...
  const codigosLista = [4,5,6,7,8,9].map(i=>f[i]).filter(v=>v && v.trim());
      ...
      <div className="md dtl-md">
          ...
          <div className="dtl-hleft">
            ...
            <div className="dtl-htitles">
              ...
              <div className="dtl-hrow2">
                ...
                <span className="dtl-verif-pill" style={estaVerificado?undefined:{background:'#546E7A',cursor:(!verifsLoaded||verifSaving||yaVerifique)?'default':'pointer'}}>
                  <VerifBadge
        ...
        <div className="dtl-body">
          <div className="dtl-grid2">
            ...
            {/* Columna derecha — código de repuesto y sus variantes */}
            <div className="dtl-codes-card">
              <div className="dtl-codes-title">
                <span className="dtl-codes-ic"><Icon.Doc/></span>Código de Repuesto
              </div>
              {codigosLista.length === 0 && (
                <div style={{fontSize:'0.68rem',color:'var(--g5)',fontStyle:'italic',padding:'4px 2px'}}>Sin códigos registrados</div>
              )}
              {codigosLista.map((c,i) => (
                <div key={i+c} className="dtl-code-row">
                  <span className="dtl-code-num">{i+1}</span>
                  <span className="dtl-code-val">{c}</span>
                  <button className="dtl-code-copy" onClick={()=>copyVal(c)} title="Copiar"><Icon.Copy/></button>
                </div>
              ))}
            </div>
          </div>

          {/* Campos adicionales — ancho completo, compactos */}
          {[
            { icon:<Icon.Doc/>,  lbl:'Descripción',      val:f[10] },
            { icon:<Icon.Doc/>,  lbl:'Modelo Original',  val:f[2] },
            { icon:<Icon.Link/>, lbl:'Equivalencia',     val:'' },
          ].map(it => (
            <div key={it.lbl} className="dtl-item compact">
              <div className="dtl-item-ic" style={{width:26,height:26}}>{it.icon}</div>
              {it.val ? (
                <div className="dtl-item-body">
                  <div className="dtl-item-lbl">{it.lbl}</div>
                  <div className="dtl-item-val">{it.val}</div>
                </div>
              ) : (
                <div className="dtl-item-body" style={{display:'flex',alignItems:'baseline',gap:8}}>
                  <span className="dtl-item-lbl" style={{marginBottom:0}}>{it.lbl}</span>
                  <span className="dtl-item-val empty" style={{fontSize:'0.72rem'}}>—</span>
                </div>
              )}
            </div>
          ))}
