...
function ClasiIcon({ clasi, sub, size=56, forceSvg=false }) {
  // Regla: la imagen debe salir si CUALQUIERA de los dos campos hace match
  // (Subclasificación O Clasificación) — no se exige que ambos coincidan a
  // la vez. Esto aplica para todas las categorías (Baterías, Filtros, etc.),
  // no solo para un caso puntual. Se prueban ambos mapas de forma
  // independiente y se usa el primero que dé resultado:
  //   1) Subclasificación específica (más precisa)
  //   2) Clasificación general
  //   3) Genérica "Todas" (repuesto.png) solo si ninguno de los dos hay match
  // El match es tolerante a mayúsculas/acentos/espacios porque el valor real
  // viene de la base de datos y puede no coincidir letra a letra con la clave.
  // forceSvg=true se salta por completo las imágenes de /public y siempre
  // dibuja el ícono vectorial (usado en contextos chicos como la píldora de
  // Novedades, donde no se quieren las fotos de fondo).
...
.cr-pill{
  ...
  background:var(--bd);
  padding:2px 10px;border-radius:6px;display:inline-flex;align-items:center;white-space:nowrap;
  letter-spacing:.03em;
}
.btn-copy{
  padding:1px 5px;
  background:rgba(255,255,255,.14);
  border:1px solid rgba(255,255,255,.22);border-radius:4px;font-size:0.62rem;cursor:pointer;color:#fff;margin-left:6px;
...
.btn-copy:active{background:rgba(255,255,255,.36)}
...
.novf-cat{display:flex;flex-direction:column;line-height:1.2;overflow:hidden}
.novf-cat b{font-size:0.6rem;letter-spacing:.2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.novf-cat small{font-size:0.52rem;opacity:.85;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.novf-chev{display:inline-block;font-size:0.85rem;font-weight:700;transition:transform .2s ease;flex-shrink:0}
...
.novf-card{
  ...
  box-shadow:0 10px 26px rgba(0,0,0,.18);padding:2px 12px;width:230px;max-width:80vw;
  cursor:pointer;
}
.novf-tagline{font-size:0.52rem;font-weight:800;letter-spacing:.5px;color:var(--gold-d);padding:7px 0 0}
...
function CatalogoApp() {
  ...
  // Cada 5s: si hay varias novedades pendientes, avanza a la siguiente
          ...
          return (<>
            ...
            <div className="ac-fg2 ac-fg2-primary">
              ...
              {novedadDisplay && (()=>{
                ...
                return (
                  <div className="novf-inline-wrap" style={{gridColumn:'3 / span 2'}}>
                    <div className="novf-pill" onClick={()=>setNovedadesExpanded(v=>!v)}>
                      {kind==='base' ? <Icon.Bell style={{width:12,height:12,flexShrink:0}}/> : <Icon.Alert style={{width:12,height:12,flexShrink:0}}/>}
                      <span className="novf-veh">{vehiculo}</span>
                      <span className="novf-sep"/>
                      <span className="novf-cat-icon"><ClasiIcon clasi={clasi} sub={sub} size={12} forceSvg/></span>
                      <span className="novf-cat">
                        <b>+ {(sub||clasi||'Novedad').toUpperCase()}</b>
                        {codLine && <small>{codLine}</small>}
                      </span>
                      <Icon.X onClick={e=>{e.stopPropagation();dismissNovedad();}} style={{width:9,height:9,opacity:.75,marginLeft:1}} title="Descartar"/>
