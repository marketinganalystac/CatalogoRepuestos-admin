...
function LoginScreen() {
  ...
  return (
    <div style={{
      ...
      <style>{`
        .ac-login-wrap{ position:relative; width:min(400px,30%); }
        .ac-login-video{
        ...
        .ac-login-card{
          ...
          z-index:1; background:#fff; border-radius:16px; overflow:hidden;
          width:min(520px,90vw);
          box-shadow:0 32px 80px rgba(0,0,0,.45);
        }
        @media(max-width:900px){
          .ac-login-wrap{ width:min(400px,60%); }
          .ac-login-video{ transform:translateX(-140px); }
          .ac-login-card{ width:min(460px,90vw); }
        }
        @media(max-width:600px){
          .ac-login-wrap{ width:100%; }
          .ac-login-video{ transform:translateX(0); inset:-10px; }
          .ac-login-card{ width:100%; position:relative; top:auto; left:auto; transform:none; }
        }
      `}</style>
      {/* Wrapper que contiene el video asomando detrás y la tarjeta de login encima */}
