...
.novf-pill{
  ...
  background:linear-gradient(135deg,var(--gold),#B38900);color:#fff;
...
.novf-cat{display:flex;flex-direction:column;line-height:1.15;overflow:hidden}
.novf-cat b{font-size:0.6rem;letter-spacing:.2px}
.novf-cat small{font-size:0.52rem;opacity:.85;font-weight:500;overflow:hidden;text-overflow:ellipsis}
...
.novf-tagline{font-size:0.52rem;font-weight:800;letter-spacing:.5px;color:var(--gold);padding:7px 0 0}
...
.novf-ic{
  width:24px;height:24px;border-radius:50%;background:var(--gl);color:var(--gold);
...
.novf-gold{color:var(--gold)!important}
...
function CatalogoApp() {
  ...
  // (el registro con el id más alto, como aproximación de "el más reciente").
  const novedadFallback = useMemo(()=>{
    if (novedadActual || fallbackDismissed || !records.length) return null;
    return [...records].sort((a,b)=>(parseInt(b._id,10)||0)-(parseInt(a._id,10)||0))[0] || null;
  },[novedadActual, fallbackDismissed, records]);
                  ...
                  <div className="novf-inline-wrap" style={{gridColumn:'3 / span 2'}}>
                    <div className="novf-pill" onClick={()=>setNovedadesExpanded(v=>!v)}>
                      ...
                      <span className="novf-cat">
                        ...
                        {(codigo||modeloOriginal) && <small>{[codigo,modeloOriginal].filter(Boolean).join(' · ')}</small>}
                      </span>
