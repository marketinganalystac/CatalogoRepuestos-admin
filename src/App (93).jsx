...
function LoginScreen() {
  ...
  return (
    <div style={{
      ...
      background:'linear-gradient(115deg, #1A3F6F 0%, #0060A0 55%, #0f2c52 100%)',
      ...
      <style>{`
        ...
        .ac-login-card{
          ...
          position:relative;
          z-index:1;
          background:linear-gradient(160deg,#1A3F6F 0%,#0f2c52 100%);
          border-radius:12px;
          overflow:hidden;
...
<div style={{ display: "flex", justifyContent: "center" }}>
  ...
  />
        ...
        <div style={{padding:'30px 32px 34px'}}>
          <p style={{fontSize:'0.68rem',color:'rgba(255,255,255,.55)',marginBottom:22,textAlign:'center'}}>
            Ingresa con tu cuenta para continuar
          </p>
          <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              <label style={{fontSize:'0.55rem',color:'rgba(255,255,255,.6)',fontWeight:600,textTransform:'uppercase',letterSpacing:.8}}>
                Correo electrónico
              </label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                placeholder="usuario@empresa.com" required autoFocus
                style={{background:'rgba(255,255,255,.08)',border:'1.5px solid rgba(255,255,255,.14)',borderRadius:8,color:'#fff',
                  padding:'11px 14px',fontSize:'0.75rem',outline:'none',transition:'border-color .15s ease,background-color .15s ease'}}
                onFocus={e=>{e.target.style.borderColor='#D4A800';e.target.style.background='rgba(255,255,255,.12)'}}
                onBlur={e=>{e.target.style.borderColor='rgba(255,255,255,.14)';e.target.style.background='rgba(255,255,255,.08)'}}/>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              <label style={{fontSize:'0.55rem',color:'rgba(255,255,255,.6)',fontWeight:600,textTransform:'uppercase',letterSpacing:.8}}>
                Contraseña
              ...
              <div style={{position:'relative'}}>
                ...
                <button type="button" onClick={()=>setShowPw(p=>!p)}
                  style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',
                    background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,.5)',fontSize:'0.72rem'}}>
                  {showPw?'🙈':'👁'}
                </button>
              </div>
            </div>
            {error && (
              <div style={{background:'rgba(198,40,40,.18)',border:'1px solid rgba(198,40,40,.4)',borderRadius:7,
                padding:'9px 13px',fontSize:'0.66rem',color:'#FF8A80',display:'flex',gap:7,alignItems:'center'}}>
                ⚠ {error === 'Invalid login credentials' ? 'Correo o contraseña incorrectos' : error}
              </div>
            )}
            <button type="submit" disabled={loading}
              style={{marginTop:6,background:'#D4A800',color:'#1A3F6F',border:'none',borderRadius:8,
                padding:'12px',fontSize:'0.74rem',fontWeight:700,cursor:loading?'default':'pointer',
                opacity:loading?.65:1,transition:'background-color .15s ease',display:'flex',alignItems:'center',justifyContent:'center',gap:8,
                letterSpacing:.1}}
              onMouseEnter={e=>{if(!loading)e.currentTarget.style.background='#e6ba00'}}
              onMouseLeave={e=>{if(!loading)e.currentTarget.style.background='#D4A800'}}>
              {loading
                ? <><span style={{display:'inline-block',width:14,height:14,border:'2px solid rgba(26,63,111,.3)',
                    borderTopColor:'#1A3F6F',borderRadius:'50%',animation:'spin .75s linear infinite'}}/> Ingresando…</>
                : 'Ingresar'}
            </button>
          </form>
          <p style={{marginTop:20,fontSize:'0.58rem',color:'rgba(255,255,255,.4)',textAlign:'center'}}>
...
.ac-progress-title{
  ...
  margin-top:40px;display:flex;align-items:center;gap:9px;
}
.ac-progress-title .dot{width:6px;height:6px;border-radius:50%;background:var(--gold);
...
.ac-progress-spin{width:46px;height:46px;border:3px solid rgba(255,255,255,.15);border-top-color:var(--gold);
  border-radius:50%;margin:0 auto 30px;animation:spin .8s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}
.ac-progress-pct{
  position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);
  font-size:0.98rem;font-weight:600;color:#fff;line-height:1;
  letter-spacing:.3px;z-index:2;
}
.ac-progress-label{font-size:0.76rem;color:rgba(255,255,255,.55);font-weight:400;overflow:hidden;text-overflow:ellipsis;display:block;margin-top:14px;max-width:100%}
.ac-progress-bar-bg{
  width:950px;
  max-width:60vw;
...
.ac-progress-bar-bg::before{content:none}
.ac-progress-bar-fill{height:100%;background:var(--bm);border-radius:9px;transition:width .3s ease;position:relative}
.ac-progress-bar-fill::after{content:none}
...
function CatalogoApp() {
  ...
  return (
    ...
    <>
      ...
      {/* BARRA DE PROGRESO GLOBAL — pantalla emergente a pantalla completa.
          Video reemplazado por un círculo de carga simple. El código del video
          queda comentado abajo por si se quiere reactivar más adelante. */}
      <div className={`ac-progress-overlay${loadProgress.active?' active':''}`}>
        <div className="ac-progress-card">
          <div className="ac-progress-spin"/>
          {/*
          <div className="ac-progress-video-wrap">
            <video
          ...
          */}
          ...
          <div className="ac-progress-title"><span className="dot"/> Cargando catálogo</div>
