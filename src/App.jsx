...
function ClasiIcon({ clasi, sub, size=56 }) {
  ...
  // buscamos en la subclasificación) > ícono SVG genérico.
  const imgSrc = SUBCLASI_IMAGES[sub] || CLASI_IMAGES[clasi];
  if (imgSrc) {
    return <img src={imgSrc} alt={sub || clasi} className="ac-clasi-img" draggable={false} />;
  }
...
.mh h2{font-size:0.82rem;color:#fff;font-weight:600}
.mx{
...
function CatalogoApp() {
  ...
  return (
    ...
    <>
      ...
      {/* FILTROS */}
      <div className={`ac-sp${(SUBCLASI_IMAGES[fSub] || CLASI_IMAGES[fClasi]) ? ' has-clasi-img' : ''}`}>
        {(() => {
