...
function ClasiIcon({ clasi, sub, size=56 }) {
  ...
  const imgSrc = SUBCLASI_IMAGES[sub] || CLASI_IMAGES[clasi] || (!clasi && !sub ? '/repuesto.png' : null);
  if (imgSrc) {
...
.ac-clasi-icon-panel img.ac-clasi-img{
  opacity:.12 !important ;position:absolute;right:0;top:50%;transform:translateY(-50%);
  width:auto;height:150%;max-height:280px;max-width:420px;object-fit:contain;z-index:0;
}
...
function CatalogoApp() {
  ...
  return (
    ...
    <>
      ...
      {/* FILTROS */}
      <div className={`ac-sp${(SUBCLASI_IMAGES[fSub] || CLASI_IMAGES[fClasi] || (!fSub && !fClasi)) ? ' has-clasi-img' : ''}`}>
        {(() => {
