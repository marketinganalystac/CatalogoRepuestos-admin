...
const VerifBadge = ({ count, threshold, yaVerifique, loading, onClick, size = 26 }) => {
  ...
  return (
    ...
    >
      ...
      {/* palomita 1 */}
      <path d="M6.9 14.6l2.6 2.6 3.4-5.2" fill="none" stroke={check1?bright:dim} strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter"/>
      {/* palomita 2 */}
...
const ModalDetail = ({ record, onClose, onEdit, onReported, onVerified }) => {
  ...
  const codigoPrincipal = f[4] && f[4].trim() ? { label: CODIGO_LABELS[4], val: f[4] } : null;
  const codigosLista = [5,6,7,8,9]
  ...
  return (
    <div className="mo show">
      <div className="md dtl-md">
        ...
        <div className="dtl-body dtl-body-split">
          ...
          <div className="dtl-codes-footer dtl-codes-aside">
            <div className="dtl-codes-footer-title">
              <Icon.Doc style={{width:13,height:13}}/>Códigos
            </div>
        ...
        <div className="mf" style={{justifyContent:'space-between',alignItems:'center'}}>
          {codigoPrincipal ? (
            <div className="dtl-code-chip" style={{margin:0}}>
              <span className="lbl">{codigoPrincipal.label}</span>
              <span className="val">{codigoPrincipal.val}</span>
              <button onClick={()=>copyVal(codigoPrincipal.val)} title="Copiar"><Icon.Copy/></button>
            </div>
          ) : <span/>}
              ...
              {paginated.map((rec,ri)=>{
                ...
                const cell={
                  ...
                  4:()=>f[4]?<span className="cr-pill">{highlightText(f[4],debText)}
                    {rec.verificado && <span title={`Verificado por ${VERIF_THRESHOLD}+ usuarios`} style={{color:'#0060A0',marginLeft:4,fontWeight:700}}>✔✔</span>}
