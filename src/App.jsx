...
function LoginScreen() {
  ...
  return (
    <div style={{
      ...
      background:'linear-gradient(135deg, #0B2A4A 0%, #0B63C4 55%, #082038 100%)',
      ...
      <div className="ac-login-outer">
        ...
        {/* Header */}
        <div style={{background:'linear-gradient(135deg,#0B2A4A,#0B63C4)',padding:'28px 32px 22px',
          borderBottom:'3px solid #F59E0B',textAlign:'center'}}>
        ...
        <div style={{padding:'28px 32px 32px'}}>
          ...
          <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{display:'flex',flexDirection:'column',gap:4}}>
              <label style={{fontSize:'0.55rem',color:'#0B63C4',fontWeight:700,textTransform:'uppercase',letterSpacing:.6}}>
                Correo electrónico
              </label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                ...
                onFocus={e=>e.target.style.borderColor='#0B63C4'}
            ...
            <button type="submit" disabled={loading}
              style={{marginTop:4,background:'#0B63C4',color:'#fff',border:'none',borderRadius:8,
...
function ToastProvider({ children }) {
  ...
  const bgMap = { success:'#2E7D32', error:'#C62828', info:'#0B63C4', warning:'#D84315' };
...
const STYLES = `
:root{
  --bd:#0B2A4A;--bm:#0B63C4;--bl:#E7F1FB;--gold:#F59E0B;--gl:#FFF3DC;
  --g1:#F8FAFC;--g2:#E2E8F0;--g3:#CBD5E1;--g5:#64748B;--g7:#334155;--g9:#0F172A;
  --red:#DC2626;--grn:#16A34A;--org:#EA580C;
  /* ── depth tokens ── */
...
/* ══════════════════════════════════
   HEADER — dos filas: identidad arriba, acciones abajo
══════════════════════════════════ */
.ac-header{
  background:var(--bd);
  padding:10px 24px;border-bottom:none;
  box-shadow:0 1px 0 rgba(255,255,255,.06) inset, 0 2px 10px rgba(0,0,0,.18);
  display:flex;flex-direction:column;gap:10px;
  position:relative;z-index:100;
}
.ac-header::after{display:none}
.ac-hl{display:flex;align-items:center;gap:14px;width:100%}
.ac-hdiv{width:1px;height:26px;background:rgba(255,255,255,.18)}
...
.ac-htitle .s2{font-size:0.85rem;color:#fff;font-weight:700;display:block}
.ac-badge{
  background:rgba(255,255,255,.08);color:#fff;font-size:0.6rem;font-weight:700;
...
/* ══════════════════════════════════
   BUTTONS — plano, moderno, sin degradados 3D
══════════════════════════════════ */
.btn{
  padding:8px 15px;border:1.5px solid transparent;border-radius:7px;font-size:0.66rem;font-weight:700;
  cursor:pointer;transition:background-color .12s ease,border-color .12s ease,color .12s ease;
  white-space:nowrap;position:relative;letter-spacing:.02em;
...
/* Primary — azul plano */
.btn-p{background:var(--bm);color:#fff;border-color:var(--bm)}
.btn-p:hover{background:#084C99;border-color:#084C99}
.btn-p:active{background:#063E80}

/* Ámbar */
.btn-g{background:var(--gold);color:#1a1200;border-color:var(--gold)}
.btn-g:hover{background:#D97706;border-color:#D97706}
.btn-g:active{background:#B45309}

/* Fantasma (header, sobre fondo oscuro) */
.btn-c{background:transparent;color:#fff;border-color:rgba(255,255,255,.28)}
.btn-c:hover{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.4)}
.btn-c:active{background:rgba(255,255,255,.14)}

/* Rojo */
.btn-r{background:var(--red);color:#fff;border-color:var(--red)}
.btn-r:hover{background:#B91C1C;border-color:#B91C1C}
.btn-r:active{background:#991B1B}

/* Outline (sobre fondo claro) */
.btn-o{background:#fff;border:1.5px solid var(--bm);color:var(--bm)}
.btn-o:hover{background:var(--bl);border-color:#084C99}
.btn-o:active{background:#D3E9FB}

/* Dark */
.btn-dark{background:var(--bd);color:#fff;border-color:var(--bd)}
.btn-dark:hover{background:#0F3358}
.btn-dark:active{background:#071C30}

/* Naranja */
.btn-org{background:var(--org);color:#fff;border-color:var(--org)}
.btn-org:hover{background:#C2410C}
.btn-org:active{background:#9A3412}

/* Slate */
.btn-slate{background:var(--g7);color:#fff;border-color:var(--g7)}
.btn-slate:hover{background:#475569}
.btn-slate:active{background:var(--g9)}

.btn-sm{padding:5px 10px;font-size:0.58rem;border-radius:6px}
.btn:disabled{opacity:.42;cursor:default;transform:none!important;box-shadow:none!important;filter:none!important}

/* Inline edit/del */
.btn-edit{
  padding:4px 10px;
  background:linear-gradient(180deg,#2C86E8 0%,var(--bm) 100%);
...
.btn-edit:hover{background:linear-gradient(180deg,#4E9CF0 0%,#0B63C4 100%);box-shadow:var(--sh-hover);transform:translateY(-1px)}
...
select,input[type=text]{
  background:#fff;border:1.5px solid var(--g3);color:var(--g9);
  padding:8px 10px;border-radius:7px;font-size:0.69rem;outline:none;transition:border-color .15s,box-shadow .15s;width:100%;
  box-shadow:none;
}
select:focus,input[type=text]:focus{
  border-color:var(--bm);
  box-shadow:0 0 0 3px rgba(11,99,196,.12);
}
...
.ac-sb{
  background:linear-gradient(135deg,var(--bd) 0%,#153A5E 100%);
...
thead th{
  background:linear-gradient(135deg,var(--bd) 0%,#082038 100%);
...
thead th:hover,thead th.sorted{background:linear-gradient(135deg,#2C86E8 0%,var(--bm) 100%)}
...
.dec-pg-toggle.active{
  background:linear-gradient(180deg,#2C86E8 0%,var(--bm) 100%)!important;
...
.pb:hover{
  background:linear-gradient(180deg,var(--bl) 0%,#D3E9FB 100%);
...
/* Progress overlay — pantalla emergente a pantalla completa (rediseño: anillo
   circular de progreso con logo al centro, sin video) */
.ac-progress-overlay{
  position:fixed;inset:0;z-index:9999;
  display:flex;align-items:center;justify-content:center;
  background:
    radial-gradient(circle at 50% 40%, rgba(90,96,106,.9) 0%, rgba(38,41,46,.96) 68%),
...
.ac-progress-ring.spin{animation:ring-spin 1.4s linear infinite}
@keyframes ring-spin{to{transform:rotate(270deg)}}
.ac-progress-ring-track{
  fill:none;stroke:rgba(255,255,255,.12);stroke-width:6;
}
.ac-progress-ring-fill{
  fill:none;stroke:url(#ac-ring-grad);stroke-width:6;stroke-linecap:round;
  transition:stroke-dashoffset .35s ease;
  filter:drop-shadow(0 0 6px rgba(79,172,219,.65));
}
.ac-progress-logo{
...
@keyframes logo-pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.045)}}
.ac-progress-title{
...
.ac-progress-pct{
  font-size:1.9rem;font-weight:800;color:#fff;line-height:1;letter-spacing:.3px;
  margin-bottom:10px;text-shadow:0 1px 6px rgba(0,0,0,.4);
}
.ac-progress-pct small{font-size:1.05rem;font-weight:700;color:#9fc9e6;margin-left:2px}
.ac-progress-label{font-size:0.78rem;color:rgba(255,255,255,.6);font-weight:500;overflow:hidden;text-overflow:ellipsis;display:block;margin-top:10px;max-width:340px;letter-spacing:.2px}
...
.dec-btn{
  background:linear-gradient(180deg,#FBBF24 0%,var(--gold) 50%,#B45309 100%);
...
.dec-btn:hover{background:linear-gradient(180deg,#FCD34D 0%,#92400E 100%);box-shadow:var(--sh-hover);transform:translateY(-1px)}
...
.badge-oem{background:linear-gradient(135deg,#E7F1FB,#D3E9FB);color:var(--bm);border:1px solid #90CAF9}
...
.seg-blue{color:var(--bm);background:linear-gradient(135deg,#E7F1FB,#D3E9FB);border-color:#90CAF9}
...
.dec-chip:hover{border-color:var(--bm);background:linear-gradient(135deg,var(--bl),#D3E9FB);box-shadow:0 3px 8px rgba(0,96,160,.15);transform:translateY(-1px)}
...
.dec-comp-btn{
  ...
  background:linear-gradient(180deg,#2C86E8 0%,var(--bm) 50%,#084C99 100%);
...
.dec-comp-btn:hover{background:linear-gradient(180deg,#4E9CF0 0%,#0B63C4 100%);box-shadow:var(--sh-hover);transform:translateY(-1px)}
...
.dec-db-btn:hover{border-color:var(--bm);background:linear-gradient(180deg,var(--bl),#D3E9FB);box-shadow:0 3px 8px rgba(0,96,160,.15);transform:translateY(-1px)}
      ...
      <div className="md">
        ...
        <div className="mb">
          ...
          <div style={{
            ...
            border: `1.5px solid ${target==='contingencia' ? '#F59E0B' : '#0B63C4'}`
          }}>
...
function CatalogoApp() {
  ...
  const fbDotColor = {connecting:'#F59E0B',ok:'#4CAF50',error:'#C62828'}[fbStatus]||'#F59E0B';
  ...
  return (
    ...
    <>
        ...
        <div className="ac-hl">
          ...
          {isAdmin ? (
            <div style={{position:'relative',display:'inline-block'}}>
              <button
                ...
                style={{
                  ...
                  borderColor: isPreviewing ? '#C62828' : (dataSource==='contingencia' ? '#F59E0B' : '#0B63C4'),
                  color: dataSource==='contingencia' ? '#92400E' : '#0B63C4',
                      ...
                      <>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background:'var(--bm,#0B63C4)',color:'#fff',
                          }}>
            ...
            <span className="fb-badge" style={{
              ...
              color: dataSource==='contingencia' ? '#92400E' : undefined,
            }}>
      ...
      {/* BARRA DE PROGRESO GLOBAL — pantalla emergente a pantalla completa.
          El video se muestra completo y grande (sin recortes). Un degradado
          interior (sombra transparente) suaviza sus bordes para que se funda
          con la tarjeta. El % va sobrepuesto dentro de la barra, que ahora es
          más gruesa, y el título "Cargando catálogo" va debajo de la barra.
          REDISEÑO: anillo de progreso circular con el logo en el centro,
          reemplaza el video (evita el defecto de recorte de fondo). */}
      <div className={`ac-progress-overlay${loadProgress.active?' active':''}`}>
        <div className="ac-progress-card">
          <div className="ac-progress-ring-wrap">
            <svg className={`ac-progress-ring${loadProgress.indeterminate?' spin':''}`} viewBox="0 0 120 120">
              <defs>
                <linearGradient id="ac-ring-grad" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#1c5a91"/>
                  <stop offset="55%" stopColor="#2A8FD8"/>
                  <stop offset="100%" stopColor="#5fb8e8"/>
                </linearGradient>
              </defs>
              <circle className="ac-progress-ring-track" cx="60" cy="60" r="54"/>
              <circle
                className="ac-progress-ring-fill"
                cx="60" cy="60" r="54"
                style={{
                  strokeDasharray: 339.3,
                  strokeDashoffset: loadProgress.indeterminate
                    ? 339.3 * 0.72
                    : 339.3 - (339.3 * Math.min(loadProgress.pct,100) / 100)
                }}
              />
            </svg>
            <img className="ac-progress-logo" src={CLASI_IMG_BASE + 'logo.png'} alt="Auto Centro" draggable={false}/>
          </div>
          {!loadProgress.indeterminate && <span className="ac-progress-pct">{loadProgress.pct}<small>%</small></span>}
      ...
      {/* FILTROS */}
      <div className={`ac-sp${CLASI_IMAGES[fClasi] ? ' has-clasi-img' : ''}`}>
        {(() => {
      ...
      <div className="ac-sb">
        ...
        <span style={{marginLeft:'auto',display:'flex',gap:5,flexWrap:'nowrap',alignItems:'center'}}>
          <span className="ac-badge" style={{marginRight:2}}>{records.length.toLocaleString()} registros</span>
      ...
      <div className="ac-tw">
        {loading ? (
          ...
          <table>
            <thead><tr>
              <th style={{width:80,minWidth:70,background:'linear-gradient(135deg,var(--bd) 0%,#082038 100%)'}}>Acciones</th>
...
function AuthGate() {
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',
      background:'linear-gradient(135deg,#0d2a4a,#0B2A4A)',fontFamily:"'Segoe UI',Arial,sans-serif"}}>
      <div style={{textAlign:'center',color:'#fff'}}>
        <div style={{width:36,height:36,border:'3px solid rgba(255,255,255,.2)',borderTopColor:'#F59E0B',
