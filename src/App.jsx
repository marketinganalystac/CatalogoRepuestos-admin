...
const COL_DEFS_ORDER = [0,1,3,13,4,5,6,7,8,9,10,12,11,2];
...
function ClasiIcon({ clasi, sub, size=56 }) {
  // Prioridad: imagen de la Subclasificación específica > imagen de la
  // Clasificación general (cuando esta es genérica y no tiene imagen propia,
  // buscamos en la subclasificación) > imagen genérica "Todas" (repuesto.png,
  // cuando no hay clasificación ni subclasificación seleccionada) > ícono SVG genérico.
  ...
  const imgSrc = findByNormalizedKey(SUBCLASI_IMAGES, sub) || findByNormalizedKey(CLASI_IMAGES, clasi)
    || (!clasi && !sub ? '/repuesto.png' : null);
  if (imgSrc) {
    // Algunos archivos en /public fueron subidos con tilde en el nombre
    // (ej. "baterías.png") mientras el mapa referencia la versión sin tilde
    // ("baterias.png"), o viceversa — según el sistema de archivos del host
    // esto puede no hacer match. onError intenta variantes con/sin acentos
    // antes de caer al ícono genérico "repuesto.png".
    const handleImgErr = e => {
      const el = e.target;
      const tried = el.dataset.triedVariant;
      if (!tried) {
        el.dataset.triedVariant = '1';
        const withTilde = imgSrc
          .replace(/baterias/i, 'baterías')
          .replace(/Baterias/i, 'Baterías');
        const withoutTilde = imgSrc
          .replace(/baterías/i, 'baterias')
          .replace(/Baterías/i, 'Baterias');
        el.src = imgSrc === withTilde ? withoutTilde : withTilde;
      } else if (tried === '1') {
        el.dataset.triedVariant = '2';
        el.src = '/repuesto.png';
      }
    };
