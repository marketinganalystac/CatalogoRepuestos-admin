...
// ── Grupos de Clasificación que comparten (o no) el universo Marca/Modelo ──
// Todo lo que NO esté listado explícitamente cae en el grupo 'autopartes'
// (Correas, Filtros, Suspensión, Frenos, etc. → Marca/Modelo = vehículo real).
// 'BATERÍAS' tiene su propio catálogo de Marca/Modelo, independiente del de
// vehículo, así que va en un grupo aparte.
const GRUPOS_VEHICULO = {
  'BATERÍAS': 'bateria',
};
const grupoDeClasi = clasi => clasi ? (GRUPOS_VEHICULO[clasi] || 'autopartes') : null;
...
function CatalogoApp() {
  ...
  // Al cambiar Clasificación/Subclasificación, Marca/Modelo/Período solo se
  // limpian si el nuevo valor pertenece a un grupo distinto al actual
  // (ej. de "Correa" a "Batería"). Dentro del mismo grupo (ej. Suspensión →
  // Correa) el vehículo seleccionado se mantiene tal cual.
  const onClasiChange  = v=>{
    const clean = cleanFilterVal(v);
    if(grupoDeClasi(clean) !== grupoDeClasi(fClasi)){
      setFMarca(''); setFModelo(''); setFPeriodo('');
    }
    setFClasi(clean); setPage(1);
  };
