...
function CatalogoApp() {
  ...
  const [fDescEstandar, setFDescEstandar] = useState('');
  ...
  // ── Novedades: últimas marcas/modelos/códigos agregados, con badge tipo
  //    "X pendientes" que el usuario puede abrir y hacer clic para ir
  //    directo al registro (igual al patrón de notificaciones de la app) ──
  ...
  const novedadesPendientes = useMemo(()=>
    novedades.filter(n => !novedadesSeenAt || new Date(n.created_at||0) > new Date(novedadesSeenAt)).length,
  ...
  const toggleNovedades = ()=>{
    setNovedadesOpen(v=>{
      const next = !v;
      if (next) {
        // Al abrir el panel, se marcan todas como vistas (baja el contador)
  ...
  // Click en una novedad: navega directo al dato — limpia otros filtros y
  // filtra por Marca+Modelo del registro para que aparezca de inmediato.
  const goToNovedad = (n)=>{
    const resumen = String(n.resumen||'');
    const marca = resumen.split(' ')[0] || '';
    setFClasi(''); setFSub(''); setFDescEstandar(''); setFLitraje(''); setFPeriodo('');
    setFModelo('');
    setFMarca(cleanFilterVal(marca));
    setFText(resumen); setDebText(resumen);
    setPage(1);
  ...
  const applyActiveFilters = useCallback((list, exclude)=>{
    ...
    if(exclude!=='desc'    && fDescEstandar) r = r.filter(x=>cleanFilterVal(x.fields[10])===fDescEstandar);
    return r;
  },[fMarca,fModelo,fLitraje,fPeriodo,fClasi,fSub,fDescEstandar]);
  ...
  const availableDescEstandar = useMemo(()=>
    [...new Set(applyActiveFilters(records,'desc').map(r=>cleanFilterVal(r.fields[10])).filter(Boolean))].sort(),
    [records,applyActiveFilters]);
  ...
  useEffect(()=>{
    if(fDescEstandar && !availableDescEstandar.includes(fDescEstandar)) setFDescEstandar('');
  },[availableDescEstandar]); // eslint-disable-line
  ...
  const filtered = useMemo(()=>{
    ...
    if(fDescEstandar) r=r.filter(x=>cleanFilterVal(x.fields[10])===fDescEstandar);
    ...
    return r;
  },[records,fMarca,fModelo,fPeriodo,fClasi,fSub,fDescEstandar,fLitraje,debText,sortCol,sortAsc,searchIndex]);
  ...
  const onDescEstandarChange = v=>{setFDescEstandar(cleanFilterVal(v));setPage(1);};
  const clearAll = ()=>{
    setFMarca('');setFModelo('');setFPeriodo('');setFClasi('');setFSub('');setFDescEstandar('');setFLitraje('');
  ...
  return (
    ...
    <>
        ...
        {(() => {
          ...
          return (<>
            ...
            <div className="ac-fg2 ac-fg2-primary">
              ...
              {renderFilter({label:'Desc. Estándar', icon:<Icon.Tag style={{width:11,height:11}}/>, val:fDescEstandar, set:onDescEstandarChange, opts:availableDescEstandar, placeholder:'Todas'})}
            </div>
      ...
      <div className="ac-sb">
        ...
        <span style={{marginLeft:'auto',display:'flex',gap:5,flexWrap:'nowrap',alignItems:'center',position:'relative'}}>
          {novedades.length>0 && (
            <>
              <button
                type="button"
                onClick={toggleNovedades}
                title="Ver últimas marcas, modelos y códigos agregados"
                style={{
                  display:'inline-flex',alignItems:'center',gap:6,
                  background:'var(--gold)',color:'var(--bd)',border:'none',
                  borderRadius:999,padding:'4px 11px',fontSize:'0.75rem',fontWeight:700,
                  cursor:'pointer',whiteSpace:'nowrap'
                }}>
                <Icon.Bell style={{width:12,height:12}}/>
                Novedades{novedadesPendientes>0?`: ${novedadesPendientes} nueva${novedadesPendientes===1?'':'s'}`:''}
              </button>
              {novedadesOpen && (
                <div style={{
                  position:'absolute',top:'calc(100% + 6px)',right:0,zIndex:50,
                  background:'#fff',color:'var(--bd)',borderRadius:10,
                  boxShadow:'0 8px 28px rgba(0,0,0,.22)',width:300,maxHeight:340,
                  overflowY:'auto',border:'1px solid rgba(0,0,0,.08)'
                }}>
                  <div style={{padding:'9px 12px',fontWeight:700,fontSize:'0.78rem',borderBottom:'1px solid rgba(0,0,0,.08)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <span>Últimas novedades</span>
                    <button type="button" onClick={()=>setNovedadesOpen(false)} style={{background:'none',border:'none',cursor:'pointer',opacity:.5,display:'flex'}}><Icon.X/></button>
                  </div>
                  {novedades.length===0 ? (
                    <div style={{padding:14,fontSize:'0.78rem',opacity:.6}}>Sin novedades recientes.</div>
                  ) : novedades.map((n,i)=>(
                    <div key={n.id||i}
                      onClick={()=>goToNovedad(n)}
                      style={{
                        padding:'9px 12px',fontSize:'0.78rem',cursor:'pointer',
                        borderBottom:i<novedades.length-1?'1px solid rgba(0,0,0,.06)':'none',
                        display:'flex',flexDirection:'column',gap:2
                      }}
                      onMouseEnter={e=>e.currentTarget.style.background='rgba(0,0,0,.04)'}
                      onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <span style={{fontWeight:600}}>{n.resumen}</span>
                      <span style={{opacity:.55,fontSize:'0.7rem'}}>{n.fecha} {n.hora}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {fDescEstandar &&<span className="ac-tag" style={{background:'rgba(255,255,255,.25)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Tag style={{width:11,height:11}}/>{fDescEstandar}</span>}
