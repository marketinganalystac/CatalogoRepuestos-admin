...
function AuthProvider({ children }) {
  ...
  const setSeccion = (s) => { seccionRef.current = s; };
...
// Nota: no cerramos la sesión al cerrar la pestaña ("beforeunload") porque
// las llamadas a Supabase requieren headers de autenticación que el
// navegador no garantiza enviar en ese momento (no es confiable en todos
// los navegadores). Por eso "¿está conectado ahora?" se calcula por
// last_seen_at reciente (umbral SESSION_ONLINE_THRESHOLD_MS), no por un
// evento de cierre explícito. logout_at solo se llena con un cierre de
// sesión real (botón "Salir").
    ...
    <div className="mo show">
      <div className="md sm">
        ...
        <div className="mf">
          ...
          <button className="btn btn-p" onClick={()=>{onClose();onEdit(record);}}><Icon.Pencil style={{marginRight:6,verticalAlign:-2}}/>Editar</button>
        </div>
      </div>
    </div>
  );
...
const ModalHistory = ({ changelog, onClose }) => {
  ...
  const opIcon={AGREGAR:<Icon.Check/>,EDITAR:<Icon.Pencil/>,ELIMINAR:<Icon.Trash/>,IMPORTAR:<Icon.Folder/>};
...
const ModalSessions = ({ sessions, onClose }) => {
  ...
  const duracion = s => {
    ...
    const fin = s.logout_at ? new Date(s.logout_at).getTime() : now;
              ...
              <div key={s.id} className="hlog-item">
                ...
                <div className="hlog-det">
                  ...
                  <span className="field-chg">
                    <em>Salida:</em> {fmt(s.logout_at)} · <em>Duración:</em> {duracion(s)}
                  </span>
                </div>
              </div>
            ))
...
function CatalogoApp() {
  ...
  // Reportar qué está buscando el usuario para el panel de "usuarios conectados"
  // (antes se reportaba solo la sección — Catálogo/Decodificador — pero eso no
  // decía qué necesitaba cada usuario; ahora se envía el término de búsqueda activo).
  useEffect(()=>{
    const termino = (showDec ? selectedCode : debText) || '';
    setSeccion?.(termino.trim() ? `Buscando: ${termino.trim()}` : null);
  }, [showDec, debText, selectedCode, setSeccion]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pendientes > 0 && !reportesAlertShown.current) {
        reportesAlertShown.current = true;
        toast(`⚠️ Tienes ${pendientes} reporte${pendientes===1?'':'s'} pendiente${pendientes===1?'':'s'} por revisar.`, 'warning');
      }
    })();
