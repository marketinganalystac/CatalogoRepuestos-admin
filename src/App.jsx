...
// Trae solo las últimas altas ('AGREGAR') del changelog — liviano, para el
// badge de "Novedades" en el arranque (no carga el historial completo).
...
function CatalogoApp() {
  ...
  // ── Novedades: última marca/modelo/código agregado manualmente.
  ...
  // Solo se muestran las novedades que el usuario aún no vio
  ...
  // Cruza la novedad (que solo trae resumen + código en el changelog) con el
  // registro completo actual para poder mostrar Clasificación/Subclasificación,
  // Especificación, Desc. Estándar y Modelo Original en la ficha expandida.
  const novedadRecord = useMemo(()=>{
    if (!novedadActual) return null;
    const codigo = novedadActual.cambios?.[0]?.despues || '';
    return (codigo && records.find(r=>r.fields[4]===codigo)) || null;
  },[novedadActual, records]);

  const dismissNovedad = ()=>{
    if (!novedadActual) return;
  ...
  // filtros y filtra por Marca+Modelo (o código, si lo tenemos) para que
  // aparezca de inmediato en la tabla.
  const goToNovedad = ()=>{
    if (!novedadActual) return;
    setFClasi(''); setFSub(''); setFLitraje(''); setFPeriodo('');
    setFModelo('');
    if (novedadRecord) {
      setFMarca(cleanFilterVal(novedadRecord.fields[0]));
      setFText(novedadRecord.fields[4]); setDebText(novedadRecord.fields[4]);
    } else {
      const marca = String(novedadActual.resumen||'').split(' ')[0] || '';
      setFMarca(cleanFilterVal(marca));
      setFText(novedadActual.resumen); setDebText(novedadActual.resumen);
    }
    setPage(1);
    setNovedadesExpanded(false);
  };
  ...
  return (
    ...
    <>
      ...
      {/* NOVEDADES — píldora flotante con lo último agregado manualmente.
          ...
          paginador para recorrer varias novedades pendientes. */}
      {novedadActual && (()=>{
        const f = novedadRecord?.fields;
        const vehiculo = f ? `${f[0]} ${f[1]}${f[3]?` · ${f[3]}`:''}` : novedadActual.resumen;
        const sub = (f && f[12]) || '';
        const clasi = (f && f[11]) || '';
        const codigo = (f && f[4]) || novedadActual.cambios?.[0]?.despues || '';
        const specLine = f ? [f[13], f[10]].filter(Boolean).join(' · ') : '';
        const modeloOriginal = f && f[2];
        return (
            ...
            <tbody>
              {paginated.map((rec,ri)=>{
                ...
                const cell={
                  ...
                  12:()=><span className="cs">{f[12]}{f[10]&&<><br/><span className="cmo">{f[10]}</span></>}</span>,
