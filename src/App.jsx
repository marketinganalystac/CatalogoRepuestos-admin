...
function LoginScreen() {
  ...
  return (
    <div style={{
      ...
      background:'linear-gradient(to bottom, #14304B 0%, #163147 50%, #1B527E 100%)',
      ...
      <style>{`
        ...
        .ac-login-wrap{
          ...
          margin-right:-225px; z-index:0;
        }
        .ac-login-video{
          position:absolute; inset:-3px; left:auto; right:0;
          width:calc(100% - 10px); height:calc(100% + 5px);
          object-fit:cover; border-radius:25px;
          z-index:0;
          mask-image:radial-gradient(ellipse 90% 90% at 20% 50%, #000 55%, transparent 100%);
          -webkit-mask-image:radial-gradient(ellipse 90% 90% at 20% 50%, #000 55%, transparent 100%);
          filter:drop-shadow(0 0 40px rgba(0,0,0,0.45));
        }
        .ac-login-card{
          position:relative; z-index:1; background:#fff; border-radius:16px; overflow:hidden;
          width:min(400px,60vw); flex:0 0 auto;
          box-shadow:0 30px 80px rgba(0,0,0,.45);
        }
        @media(max-width:900px){
          .ac-login-wrap{ display:none; margin-right:0; }
      ...
      `}</style>
      {/* Contenedor externo tipo flex: video y card viajan juntos como una sola unidad centrada */}
      <div className="ac-login-outer">
        {/* Wrapper del video — el margin-right negativo controla cuánto "asoma" bajo el card */}
