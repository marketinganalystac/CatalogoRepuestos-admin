...
// ── Sonido "dindón" de campana (Web Audio API, sin archivos externos) ──
let _dindonCtx = null;
function playDindon() {
  try {
    const Ctx = window.AudioContext || window.webkitAudioContext;
    if (!Ctx) return;
    if (!_dindonCtx) _dindonCtx = new Ctx();
    const ctx = _dindonCtx;
    ...
    tone(880, now, 0.28);        // "din"
    tone(659.25, now + 0.22, 0.35); // "don"
  } catch { /* silencioso si el navegador bloquea audio sin interacción previa */ }
}

// ── Íconos sólidos (reemplazan emojis) — SVG inline, sin dependencias externas ──
const Icon = {
  Plus: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M11 4a1 1 0 0 1 2 0v7h7a1 1 0 0 1 0 2h-7v7a1 1 0 0 1-2 0v-7H4a1 1 0 0 1 0-2h7V4Z"/></svg>,
  Folder: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M3 6.5A1.5 1.5 0 0 1 4.5 5h5.17a1.5 1.5 0 0 1 1.06.44L12.06 6.7A1.5 1.5 0 0 0 13.12 7H19.5A1.5 1.5 0 0 1 21 8.5v9A1.5 1.5 0 0 1 19.5 19h-15A1.5 1.5 0 0 1 3 17.5v-11Z"/></svg>,
  Eye: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 5c-5.5 0-9.3 4.4-10.6 6.4a1.4 1.4 0 0 0 0 1.2C2.7 14.6 6.5 19 12 19s9.3-4.4 10.6-6.4a1.4 1.4 0 0 0 0-1.2C21.3 9.4 17.5 5 12 5Zm0 11.5a4.5 4.5 0 1 1 0-9 4.5 4.5 0 0 1 0 9Z"/></svg>,
  EyeOff: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M3.3 2.3a1 1 0 0 0-1.4 1.4l3.1 3.1C3 8.3 1.7 10 1.4 10.6a1.4 1.4 0 0 0 0 1.2C2.7 14.6 6.5 19 12 19c1.6 0 3.1-.4 4.4-1l3.3 3.3a1 1 0 0 0 1.4-1.4L3.3 2.3ZM12 16.5c-.5 0-1-.1-1.4-.3l-2-2A4.5 4.5 0 0 1 12 7.5c.5 0 1 .1 1.4.3l1.6 1.6c.2.4.3.9.3 1.4a4.5 4.5 0 0 1-3.3 4.3Zm8.6-4.7C19.3 13.7 15.5 18.1 10 18.1l-1.7-.1 1.8-1.8a4.5 4.5 0 0 0 5.6-5.6l3.4-3.4c1 1 1.9 2.2 2.5 3.1a1.4 1.4 0 0 1 0 1.2Z"/></svg>,
  Refresh: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 4a8 8 0 0 1 7.75 6h-2.1A6 6 0 0 0 12 6a6 6 0 0 0-4.9 2.5L9 10.5H3v-6l2.1 2.1A8 8 0 0 1 12 4Zm0 16a8 8 0 0 1-7.75-6h2.1A6 6 0 0 0 12 18a6 6 0 0 0 4.9-2.5L15 13.5h6v6l-2.1-2.1A8 8 0 0 1 12 20Z"/></svg>,
  History: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 3a9 9 0 1 0 9 9h-2a7 7 0 1 1-2.05-4.95L14.5 9.5H21V3l-2.6 2.6A8.96 8.96 0 0 0 12 3Zm-1 4v6l4.5 2.7.9-1.5-3.6-2.2V7H11Z"/></svg>,
  LogOut: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M10 3a1 1 0 0 1 0 2H6v14h4a1 1 0 0 1 0 2H5a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5Zm5.3 3.3a1 1 0 0 1 1.4 0l4 4a1 1 0 0 1 0 1.4l-4 4a1 1 0 0 1-1.4-1.4L17.6 12H9a1 1 0 0 1 0-2h8.6l-2.3-2.3a1 1 0 0 1 0-1.4Z"/></svg>,
  Trash: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M9 3a1 1 0 0 0-1 1v1H4.5a1 1 0 0 0 0 2H5v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7h.5a1 1 0 0 0 0-2H16V4a1 1 0 0 0-1-1H9Zm1 6a1 1 0 0 1 2 0v8a1 1 0 0 1-2 0V9Zm4 0a1 1 0 0 1 2 0v8a1 1 0 0 1-2 0V9Z"/></svg>,
  Pencil: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M4 20.5V17l10.5-10.5 3.5 3.5L7.5 20.5H4ZM19.7 6.8l-2-2a1.5 1.5 0 0 0-2.1 0l-1.4 1.4 4.1 4.1 1.4-1.4a1.5 1.5 0 0 0 0-2.1Z"/></svg>,
  X: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M6.4 4.9a1 1 0 0 0-1.4 1.4L10.6 12l-5.6 5.6a1 1 0 1 0 1.4 1.4L12 13.4l5.6 5.6a1 1 0 0 0 1.4-1.4L13.4 12l5.6-5.6a1 1 0 0 0-1.4-1.4L12 10.6 6.4 4.9Z"/></svg>,
  Check: (p) => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M20.3 5.7a1 1 0 0 1 0 1.4l-10.6 11a1 1 0 0 1-1.45.02L3.7 13.6a1 1 0 1 1 1.4-1.4l3.8 3.8L18.9 5.7a1 1 0 0 1 1.4 0Z"/></svg>,
  Download: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 3a1 1 0 0 1 1 1v9.6l3.3-3.3a1 1 0 1 1 1.4 1.4l-5 5a1 1 0 0 1-1.4 0l-5-5a1 1 0 1 1 1.4-1.4l3.3 3.3V4a1 1 0 0 1 1-1ZM5 19a1 1 0 0 1 1-1h12a1 1 0 0 1 0 2H6a1 1 0 0 1-1-1Z"/></svg>,
  Search: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M10.5 3a7.5 7.5 0 1 0 4.6 13.4l4.75 4.75a1 1 0 0 0 1.4-1.4l-4.74-4.75A7.5 7.5 0 0 0 10.5 3Zm-5.5 7.5a5.5 5.5 0 1 1 11 0 5.5 5.5 0 0 1-11 0Z"/></svg>,
  Alert: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 2 1 21h22L12 2Zm0 6.5a1 1 0 0 1 1 1v4.5a1 1 0 1 1-2 0V9.5a1 1 0 0 1 1-1ZM12 18.2a1.2 1.2 0 1 1 0-2.4 1.2 1.2 0 0 1 0 2.4Z"/></svg>,
  Copy: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M8 3a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h2v2a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2V5a2 2 0 0 0-2-2H8Zm2 12V5h6v2h-4a2 2 0 0 0-2 2v6h-.01Zm2 4V9h6v10h-6Z"/></svg>,
  Upload: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 21a1 1 0 0 1-1-1v-9.6l-3.3 3.3a1 1 0 1 1-1.4-1.4l5-5a1 1 0 0 1 1.4 0l5 5a1 1 0 1 1-1.4 1.4L13 10.4V20a1 1 0 0 1-1 1ZM5 5a1 1 0 0 1 1-1h12a1 1 0 0 1 0 2H6a1 1 0 0 1-1-1Z"/></svg>,
  Tag: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12.6 3.4a2 2 0 0 1 1.4-.6h5.5A1.5 1.5 0 0 1 21 4.3v5.5a2 2 0 0 1-.6 1.4l-9 9a2 2 0 0 1-2.8 0l-5.8-5.8a2 2 0 0 1 0-2.8l9-9Zm4.9 3.1a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0Z"/></svg>,
  Car: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M5 11 6.6 6.4A2 2 0 0 1 8.5 5h7a2 2 0 0 1 1.9 1.4L19 11h.5A1.5 1.5 0 0 1 21 12.5v4a1 1 0 0 1-1 1h-1a2 2 0 1 1-4 0H9a2 2 0 1 1-4 0H4a1 1 0 0 1-1-1v-4A1.5 1.5 0 0 1 4.5 11H5Zm2.1 0h9.8l-1.3-3.8a.5.5 0 0 0-.47-.2h-6.26a.5.5 0 0 0-.47.2L7.1 11Z"/></svg>,
  Calendar: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M7 2a1 1 0 0 1 1 1v1h8V3a1 1 0 1 1 2 0v1h1a2 2 0 0 1 2 2v13a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h1V3a1 1 0 0 1 1-1ZM5 10v9h14v-9H5Z"/></svg>,
  Ruler: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M3.5 15.4 8.6 20.5a1.7 1.7 0 0 0 2.4 0l9.5-9.5a1.7 1.7 0 0 0 0-2.4L15.4 3.5a1.7 1.7 0 0 0-2.4 0l-9.5 9.5a1.7 1.7 0 0 0 0 2.4Zm7.6-7.6 1.4 1.4-1.1 1.1-1.4-1.4 1.1-1.1Zm2.5-2.5 1.4 1.4-1.1 1.1-1.4-1.4 1.1-1.1ZM8.6 10.3 10 11.7l-1.1 1.1-1.4-1.4 1.1-1.1Z"/></svg>,
  Bookmark: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M6 3a2 2 0 0 0-2 2v15.2a1 1 0 0 0 1.55.83L12 17.1l6.45 3.93A1 1 0 0 0 20 20.2V5a2 2 0 0 0-2-2H6Z"/></svg>,
  Layers: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 2 2 7l10 5 10-5-10-5Zm0 8L4 6.5 12 3l8 3.5L12 10Zm-8 2.5 8 4 8-4V15l-8 4-8-4v-2.5Z"/></svg>,
  Bell: (p) => <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M12 2a1 1 0 0 1 1 1v1.06A7 7 0 0 1 19 11v4.1l1.7 2.55a1 1 0 0 1-.83 1.55H4.13a1 1 0 0 1-.83-1.55L5 15.1V11a7 7 0 0 1 6-6.94V3a1 1 0 0 1 1-1Zm0 19a2.5 2.5 0 0 1-2.45-2h4.9A2.5 2.5 0 0 1 12 21Z"/></svg>,
  Doc: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M6 2a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6H6Zm7 1.5L18.5 9H13V3.5ZM7 13h10v1.5H7V13Zm0 4h10v1.5H7V17Zm0-8h4v1.5H7V9Z"/></svg>,
  Link: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M10.6 13.4a1 1 0 0 1 0-1.4l3-3a3.5 3.5 0 1 1 4.95 4.95l-1.6 1.6a1 1 0 0 1-1.4-1.4l1.6-1.6a1.5 1.5 0 0 0-2.12-2.12l-3 3a1 1 0 0 1-1.43 0ZM13.4 10.6a1 1 0 0 1 0 1.4l-3 3a3.5 3.5 0 1 1-4.95-4.95l1.6-1.6a1 1 0 1 1 1.4 1.4l-1.6 1.6a1.5 1.5 0 0 0 2.12 2.12l3-3a1 1 0 0 1 1.43 0Z"/></svg>,
  Network: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" {...p}><circle cx="6" cy="6" r="2.6" fill="currentColor" stroke="none"/><circle cx="18" cy="6" r="2.6" fill="currentColor" stroke="none"/><circle cx="12" cy="18" r="2.6" fill="currentColor" stroke="none"/><path d="M8.3 7.3 11 16M15.7 7.3 13 16M8.6 5.7h6.8"/></svg>,
};
...
// ============================================================
const SUPABASE_URL = "https://vzjhzuvahejosdojllcm.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ6amh6dXZhaGVqb3Nkb2psbGNtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIyMTI3NTAsImV4cCI6MjA4Nzc4ODc1MH0.8PNlIh3HQDIq1u6IiRQeKx3o9gZyNWU3SeZ4qJ_F7Ew";
...
// Clave en sessionStorage para no duplicar el registro de ingreso si la
// pestaña simplemente se refresca (la sesión de Supabase sobrevive al
// refresh porque usa sessionStorage; el id de la fila de log debe
// sobrevivir junto con ella para no crear un "login" nuevo cada F5).
const SESSION_LOG_ID_KEY = 'ac_session_log_id';

function AuthProvider({ children }) {
  const [user,    setUser]    = useState(null);
  const [role,    setRole]    = useState(null);
  const [loading, setLoading] = useState(true);
  const sessionIdRef = useRef(null);   // id de la fila en user_sessions
  const seccionRef   = useRef(null);   // sección actual, para el latido
  const accessTokenRef = useRef(null); // JWT vigente, para poder cerrar la
                                        // sesión con un beacon al cerrar el navegador
  ...
  // Abre (o retoma) el registro de ingreso para el usuario dado
  const openSessionLog = async (authUser) => {
    let existing = null;
    try { existing = sessionStorage.getItem(SESSION_LOG_ID_KEY); } catch {}
    if (existing) {
      sessionIdRef.current = existing;
      fsSessionHeartbeat(existing, seccionRef.current); // "sigo aquí"
      return;
    }
    const id = await fsSessionStart(authUser.id, authUser.email);
    sessionIdRef.current = id;
    if (id) { try { sessionStorage.setItem(SESSION_LOG_ID_KEY, id); } catch {} }
  };

  const closeSessionLog = async () => {
    const id = sessionIdRef.current;
    sessionIdRef.current = null;
    try { sessionStorage.removeItem(SESSION_LOG_ID_KEY); } catch {}
    if (id) await fsSessionEnd(id);
  };
  ...
  // Latido: cada SESSION_HEARTBEAT_MS confirmamos "sigo activo" y con qué
  // sección, para que el panel de admin sepa quién está conectado ahora.
  useEffect(() => {
    if (!user) return;
    const iv = setInterval(() => {
      if (sessionIdRef.current) fsSessionHeartbeat(sessionIdRef.current, seccionRef.current);
    }, SESSION_HEARTBEAT_MS);

    return () => clearInterval(iv);
  }, [user]);

  // Registrar la "hora de salida" cuando el usuario cierra la pestaña o el
  // navegador (no solo cuando presiona "Salir"). Un fetch normal se
  // cancelaría al descargar la página, así que usamos keepalive para que
  // el navegador lo complete aunque la pestaña ya se esté cerrando. Esto
  // es "mejor esfuerzo": si el navegador se cierra abruptamente (o pierde
  // la red) puede no llegar a tiempo, por eso el panel de Usuarios también
  // muestra una salida aproximada basada en el último latido.
  useEffect(() => {
    const closeOnUnload = () => {
      const id = sessionIdRef.current;
      ...
      if (!id || !token) return;
      try {
        fetch(`${SUPABASE_URL}/rest/v1/${COL_SESSIONS}?id=eq.${id}`, {
          method: 'PATCH',
          keepalive: true,
          headers: {
          ...
          body: JSON.stringify({ logout_at: new Date().toISOString(), seccion: null }),
        });
      } catch {}
    };
  ...
  // Los componentes hijos llaman a esto cuando el usuario cambia de
  // sección (catálogo, decodificador, historial, reportes, etc.) para que
  // el panel de "usuarios conectados" muestre dónde está cada quien.
  // Se envía de inmediato (no se espera al latido periódico de
  // SESSION_HEARTBEAT_MS) para que el panel de admin refleje el cambio sin
  // demora perceptible.
  const setSeccion = (s) => {
    if (seccionRef.current === s) return;
    seccionRef.current = s;
    if (sessionIdRef.current) fsSessionHeartbeat(sessionIdRef.current, s);
  };

  const isAdmin = role === 'admin';

  return (
    <AuthCtx.Provider value={{ user, role, isAdmin, loading, signIn, signOut, setSeccion }}>
      {children}
    </AuthCtx.Provider>
  );
}
...
function LoginScreen() {
  ...
  return (
    <div style={{
      ...
      background:'radial-gradient(circle at 25% 20%, #0f4a85 0%, #0a2f5c 45%, #081f3f 100%)',
      ...
      <style>{`
        ...
        .ac-login-wrap{
          display:none; /* Video oculto por ahora — reactivar y personalizar más adelante */
        }
        .ac-login-video{
          position:absolute;
          top:0;
        ...
        .ac-login-card{
          ...
          box-shadow:0 24px 60px rgba(6,14,28,.45),0 4px 14px rgba(6,14,28,.25);
        }
      ...
      <div className="ac-login-outer">
        {/* Wrapper del video — oculto por ahora (display:none arriba). Se deja el código listo
            para reactivarlo más adelante con una personalización distinta.
            Video 1 y Video 2 son elementos <video> independientes, con exactamente la misma clase CSS
            (.ac-login-video) y la misma posición. Ciclo:
              1) Video 1 arranca en el segundo 0 y se muestra 4s.
              2) Se oculta y se espera 3s.
              3) Video 2 aparece arrancando exactamente en el segundo 3 y corre hasta el final.
              4) Al terminar Video 2, el ciclo reinicia con Video 1 desde el segundo 0.
            Ambos a 0.7x de velocidad. Son independientes: nunca comparten currentTime,
            así que uno no interfiere con la reproducción del otro. */}
        <div
          className="ac-login-wrap"
          ref={(wrap) => {
            if (!wrap) return;
            if (wrap.dataset.controlado) return;
            wrap.dataset.controlado = "true";

            const v1 = wrap.querySelector('[data-video="1"]');
            const v2 = wrap.querySelector('[data-video="2"]');
            if (!v1 || !v2) return;

            v1.playbackRate = 0.7;
            v2.playbackRate = 0.7;

            let ocultarV1, mostrarV2;

            const cicloV1 = () => {
              clearTimeout(ocultarV1);
              clearTimeout(mostrarV2);

              // Video 2 se detiene y se oculta; Video 1 toma el control desde el segundo 0
              v2.pause();
              ...
              // A los 4 segundos, ocultar Video 1
              ocultarV1 = setTimeout(() => {
                v1.pause();
                v1.style.display = "none";

                // 3 segundos después, mostrar Video 2 desde el segundo 3
                mostrarV2 = setTimeout(() => {
                  // Video 2 permanece oculto mientras "busca" el segundo 3;
                  // solo se muestra y reproduce cuando el seek terminó (evento "seeked"),
                  // así se evita el frame viejo que se veía por 1 segundo al iniciar.
                  v2.style.display = "none";
                  const alTerminarSeek = () => {
                    v2.removeEventListener("seeked", alTerminarSeek);
                    v2.style.display = "block";
                    v2.play();
                  };
                  v2.addEventListener("seeked", alTerminarSeek);
                  v2.currentTime = 3;
                }, 3000);
              }, 4000);
            };

            // Cuando Video 2 termina de reproducirse, reinicia el ciclo con Video 1
            v2.addEventListener("ended", cicloV1);
        ...
        {/* Header */}
        <div style={{background:'linear-gradient(115deg,#1A3F6F,#0f2c52)',padding:'32px 32px 24px',
          borderBottom:'2px solid #D4A800',textAlign:'center'}}>
<div style={{ display: "flex", justifyContent: "center" }}>
  <img 
    src={LOGO_SRC} 
    alt="Auto Centro" 
    style={{ height: 46, objectFit: "contain", marginBottom: 12 }}
  />
</div>
          <div style={{color:'rgba(255,255,255,.5)',fontSize:'0.57rem',textTransform:'uppercase',letterSpacing:1.8,fontWeight:600}}>
            Sistema de Gestión
          </div>
          <div style={{color:'#fff',fontSize:'0.9rem',fontWeight:600,marginTop:3,letterSpacing:.1}}>
            Aplicación de Repuestos
          </div>
        </div>
        {/* Form */}
        <div style={{padding:'30px 32px 34px'}}>
          <p style={{fontSize:'0.68rem',color:'#78909C',marginBottom:22,textAlign:'center'}}>
            Ingresa con tu cuenta para continuar
          </p>
          <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              <label style={{fontSize:'0.55rem',color:'#546E7A',fontWeight:600,textTransform:'uppercase',letterSpacing:.8}}>
                Correo electrónico
              </label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                placeholder="usuario@empresa.com" required autoFocus
                style={{background:'#F7F9FB',border:'1.5px solid #E1E7EC',borderRadius:8,
                  padding:'11px 14px',fontSize:'0.75rem',outline:'none',transition:'border-color .15s ease,background-color .15s ease'}}
                onFocus={e=>{e.target.style.borderColor='#0060A0';e.target.style.background='#fff'}}
                onBlur={e=>{e.target.style.borderColor='#E1E7EC';e.target.style.background='#F7F9FB'}}/>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              <label style={{fontSize:'0.55rem',color:'#546E7A',fontWeight:600,textTransform:'uppercase',letterSpacing:.8}}>
                Contraseña
              ...
              <div style={{position:'relative'}}>
                <input type={showPw?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)}
                  ...
                  style={{background:'#F7F9FB',border:'1.5px solid #E1E7EC',borderRadius:8,
                    padding:'11px 40px 11px 14px',fontSize:'0.75rem',outline:'none',width:'100%',transition:'border-color .15s ease,background-color .15s ease'}}
                  onFocus={e=>{e.target.style.borderColor='#0060A0';e.target.style.background='#fff'}}
                  onBlur={e=>{e.target.style.borderColor='#E1E7EC';e.target.style.background='#F7F9FB'}}/>
                <button type="button" onClick={()=>setShowPw(p=>!p)}
                  style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',
                    background:'none',border:'none',cursor:'pointer',color:'#90A4AE',display:'flex',alignItems:'center'}}>
                  {showPw?<Icon.EyeOff/>:<Icon.Eye/>}
                </button>
              </div>
            </div>
            {error && (
              <div style={{background:'#FDF2F2',border:'1px solid #F4CACA',borderRadius:7,
                padding:'9px 13px',fontSize:'0.66rem',color:'#C62828',display:'flex',gap:7,alignItems:'center'}}>
                <Icon.Alert style={{flexShrink:0}}/>{error === 'Invalid login credentials' ? 'Correo o contraseña incorrectos' : error}
              </div>
            )}
            <button type="submit" disabled={loading}
              style={{marginTop:6,background:'#0060A0',color:'#fff',border:'none',borderRadius:8,
                padding:'12px',fontSize:'0.74rem',fontWeight:600,cursor:loading?'default':'pointer',
                opacity:loading?.65:1,transition:'background-color .15s ease',display:'flex',alignItems:'center',justifyContent:'center',gap:8,
                letterSpacing:.1}}
              onMouseEnter={e=>{if(!loading)e.currentTarget.style.background='#0a6cad'}}
              onMouseLeave={e=>{if(!loading)e.currentTarget.style.background='#0060A0'}}>
              {loading
                ? <><span style={{display:'inline-block',width:14,height:14,border:'2px solid rgba(255,255,255,.3)',
                    borderTopColor:'#fff',borderRadius:'50%',animation:'spin .75s linear infinite'}}/> Ingresando…</>
                : 'Ingresar'}
            </button>
          </form>
          <p style={{marginTop:20,fontSize:'0.58rem',color:'#B0BEC5',textAlign:'center'}}>
...
const COL_REPORTES  = 'reportes';
const COL_SESSIONS  = 'user_sessions';
// Si no hay "latido" (heartbeat) en más de este tiempo, se considera al
// usuario desconectado aunque nunca haya cerrado sesión formalmente
// (ej. cerró la pestaña sin dar logout).
const SESSION_HEARTBEAT_MS = 30_000;
const SESSION_ONLINE_THRESHOLD_MS = 90_000;
// Nota: además del cierre explícito por el botón "Salir", intentamos
// registrar logout_at al cerrar la pestaña/navegador con un fetch
// "keepalive" en pagehide/beforeunload (mejor esfuerzo — el navegador no
// garantiza que siempre llegue, ej. cierre abrupto o sin red). Por eso
// "¿está conectado ahora?" se sigue calculando por last_seen_at reciente
// (umbral SESSION_ONLINE_THRESHOLD_MS), y el panel de Usuarios muestra una
// salida aproximada (basada en el último latido) cuando logout_at nunca
// llegó a registrarse.
...
const COL_DEFS = [
  ...
  { key:13, label:'Especificación',  show:true,  width:90  },
];
// Ordered display: Marca, Modelo, Período, Especificación, Código Repuesto,
// Código 1-3, (Código 4/5 y Desc. Estándar ocultas), Clasificación,
// Subclasificación, (Modelo Original oculta)
...
const normalizeDoc = (raw) => {
  if (!raw) return null;
  const verificado = !!raw.verificado;
  if (Array.isArray(raw.fields) && raw.fields.length === 14)
    return { _id: raw._id, verificado, fields: raw.fields.map(v => String(v ?? '')) };
...
// ── Imágenes de fondo por Clasificación (repo CatalogoRepuestos-admin/public) ──
// Cuando existe una imagen para la clasificación activa, se usa en vez del
// ícono SVG genérico. Si no hay imagen, se cae al ícono SVG (ver ClasiIcon).
const CLASI_IMG_BASE = 'https://raw.githubusercontent.com/marketinganalystac/CatalogoRepuestos-admin/main/public/';
const CLASI_IMAGES = {
  'BATERÍAS':               CLASI_IMG_BASE + 'BATERIAS.PNG',
  'FILTROS':                CLASI_IMG_BASE + 'FILTROS.PNG',
  'SUSPENSIÓN Y DIRECCIÓN': CLASI_IMG_BASE + 'BRAZO%20DE%20SUSPENCION.PNG',
  'CLUTCH Y TRANSMISIÓN':   CLASI_IMG_BASE + 'CORREAS.PNG',
  'LUBRICANTES':            CLASI_IMG_BASE + 'LUBRICANTES.PNG',
  'LLANTAS':                '/llantas.png',
};
// ── Imágenes de fondo por Subclasificación (archivos en /public) ──
// Más específicas que las de Clasificación: si hay una subclasificación
// seleccionada y tiene imagen propia, esa gana sobre la de la clasificación
// general (ver lógica de prioridad en ClasiIcon).
const SUBCLASI_IMAGES = {
  'Amortiguadores':        '/amortiguadores.png',
  'Balinera de Clutch':    '/balineras.png',
  'Balineras':             '/balineras.png',
  'Bandas de Freno':       '/bandas_de_freno.png',
  'Bases de Amortiguador': '/bases_de_amortiguador.png',
  'Baterías':              '/baterias.png',
  'Bolas / Rótulas':       '/bolas_rotulas.png',
  'Brazos / Links':        '/brazos_links.png',
  'Bujes':                 '/bujes.png',
  'Bujías':                '/bujias.png',
  'Cilindros de Freno':    '/cilindros_de_freno.png',
  'Correas':               '/correas.png',
  'Cremallera':            '/cremallera.png',
  'Discos de Freno':       '/discos_de_freno.png',
  'Filtro de Aceite':      '/filtro_de_aceite.png',
  'Filtro de Aire':        '/filtro_de_aire.png',
  'Filtro de Combustible': '/filtro_de_combustible.png',
  'Hub / Cubos':           '/hub_cubos.png',
  'Master de Freno':       '/master_de_freno.png',
  'Muñequilla / Ejes':     '/muenequilla_ejes.png',
  'Pastillas / Tacos':     '/pastillas.png',
  'Plato de Clutch':       '/plato_de_clutch.png',
  'Tambores':              '/tambores.png',
  'Tensores':              '/tensores.png',
  'Terminales / V':        '/terminales.png',
};

// Busca en un mapa de imágenes por clave "tolerante" (sin acentos, minúsculas,
// espacios/guiones normalizados) porque el valor real de Clasificación/
// Subclasificación viene de la base de datos y puede no coincidir letra a
// letra con las claves definidas arriba (mayúsculas, tildes, plural, singular,
// abreviaturas, orden de palabras, etc.).
const _normImgKeyCache = new Map();
function _normImgKey(s) {
  return String(s ?? '')
    .toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]/g, '');
...
// convertir una palabra en otra. 0 = idénticas, mientras más bajo, más parecidas.
...
function findByNormalizedKey(map, value) {
  if (!value) return null;
  let idx = _normImgKeyCache.get(map);
  if (!idx) {
    idx = {};
    for (const k in map) idx[_normImgKey(k)] = map[k];
    _normImgKeyCache.set(map, idx);
  }
  const key = _normImgKey(value);
  // 1) Match exacto
  if (idx[key]) return idx[key];
  // 2) Uno "contiene" al otro (ej. "amortiguadordelantero" contiene "amortiguador")
  for (const k in idx) {
    if (key.includes(k) || k.includes(key)) return idx[k];
  }
  // 3) La clave más parecida por distancia de edición (tolera errores de
  // tipeo, singular/plural, variaciones de escritura), solo si es
  // razonablemente cercana (evita falsos positivos entre palabras distintas).
  let best = null, bestDist = Infinity;
  for (const k in idx) {
    const d = _levenshtein(key, k);
    if (d < bestDist) { bestDist = d; best = idx[k]; }
  }
  const threshold = Math.max(2, Math.round(key.length * 0.3));
...
function ClasiIcon({ clasi, sub, size=56, forceSvg=false }) {
  // TEMPORAL: mientras se suben las imágenes correctas por categoría, se usa
  ...
  if (forceSvg) {
    ...
    // protagonismo visual lo debe llevar el texto de la subcategoría.
    return <img src={'/logoo.png'} alt={sub || clasi || 'Todas las categorías'} draggable={false}
      style={{width:size,height:size,minWidth:size,objectFit:'contain',flexShrink:0}}/>;
  }
  return <img src={TEMP_LOGO} alt={sub || clasi || 'Todas las categorías'} className="ac-clasi-img" draggable={false}/>;
  /* ---- lógica original por categoría (referencia para cuando haya fotos reales) ----
  const subMatch   = !forceSvg && findByNormalizedKey(SUBCLASI_IMAGES, sub);
  const clasiMatch = !forceSvg && findByNormalizedKey(CLASI_IMAGES, clasi);
  const imgSrc = !forceSvg && (subMatch || clasiMatch || (!clasi && !sub ? '/repuesto.png' : null));
  if (imgSrc) {
    ...
    // según el sistema de archivos / repo, esto puede no hacer match exacto.
    // onError prueba, en orden: mayúsculas, minúsculas, con tilde, sin tilde,
    // antes de caer al ícono genérico "repuesto.png".
    const buildVariants = src => {
      const withTilde = src.replace(/baterias/gi, m => m[0]==='B'?'Baterías':'baterías');
      const withoutTilde = src.replace(/bater[íi]as/gi, m => m[0]==='B'?'Baterias':'baterias');
      const bases = new Set([src, withTilde, withoutTilde]);
      const all = new Set();
      bases.forEach(b => {
        all.add(b);
        const dot = b.lastIndexOf('.');
        if (dot > -1) {
          const name = b.slice(0, dot), ext = b.slice(dot);
          all.add(name.toUpperCase() + ext.toUpperCase());
          all.add(name.toLowerCase() + ext.toLowerCase());
        }
      });
      all.delete(src);
      return [...all];
    ...
    const handleImgErr = e => {
      const el = e.target;
      let variants = el.dataset.variants ? JSON.parse(el.dataset.variants) : null;
      if (!variants) {
        variants = buildVariants(imgSrc);
        el.dataset.variants = JSON.stringify(variants);
        el.dataset.variantIdx = '0';
      }
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
      ...
      <circle cx="12" cy="12" r="11" fill="currentColor" fillOpacity=".08" stroke="none"/>
      <g strokeWidth="1.55">{inner}</g>
    </svg>
  );
  */
}
...
// ── Verificación por consenso ("doble palomita azul") ──
// Un repuesto queda "verificado" cuando VERIF_THRESHOLD usuarios distintos
// confirman que el código coincide con lo real. Se revoca automáticamente
// (por trigger en Supabase) si alguien reporta ese mismo código después.
const COL_VERIFICACIONES = 'verificaciones';
const VERIF_THRESHOLD = 2;

const fsAddVerificacion = async (repuestoId, usuario) => {
  const { error } = await supabaseSession
    .from(COL_VERIFICACIONES)
    .insert([{ repuesto_id: repuestoId, usuario }]);
  if (error) throw new Error(error.message);
};

const fsGetVerificaciones = async (repuestoId) => {
  const { data, error } = await supabaseSession
    .from(COL_VERIFICACIONES)
    .select('*')
...
// ── Reportes de usuario (código o dato erróneo) — "ticket helpdesk" ──
const fsAddReporte = async (entry) => {
  const { error } = await supabaseSession
    .from(COL_REPORTES)
    .insert([{ ...entry }]);
  if (error) throw new Error(error.message);
...
const fsGetReportes = async () => {
  const { data, error } = await supabaseSession
    .from(COL_REPORTES)
    .select('*')
    .order('created_at', { ascending: false });
  if (error) throw new Error(error.message);
  return (data || []).map(r => ({ ...r, _id: String(r.id) }));
};

const fsUpdateReporteEstado = async (id, estado, resuelto_por) => {
  const { error } = await supabaseSession
    .from(COL_REPORTES)
    .update({
      estado,
      resuelto_por: estado === 'resuelto' ? (resuelto_por || null) : null,
...
// Trae las últimas altas y ediciones ('AGREGAR' / 'EDITAR') del changelog —
// liviano, para el badge de "Novedades" en el arranque (no carga el
// historial completo).
const fsGetRecentAdds = async (limit = 20) => {
  const { data, error } = await supabaseSession
    .from(COL_CHANGELOG)
    .select('*')
    .in('op', ['AGREGAR', 'EDITAR'])
    .order('created_at', { ascending: false })
    .limit(limit);
  if (error) throw new Error(error.message);
  return data || [];
...
// ── Log de sesiones de usuario (ingreso/salida + presencia) ──
// Requiere una tabla `user_sessions` en Supabase con columnas:
//   id (uuid/serial, pk) · user_id (uuid) · email (text)
//   login_at (timestamptz) · logout_at (timestamptz, null mientras activo)
//   last_seen_at (timestamptz) · seccion (text, null si no está en ninguna)
const fsSessionStart = async (userId, email) => {
  const { data, error } = await supabaseSession
    .from(COL_SESSIONS)
    .insert([{
      user_id: userId,
    ...
    .select('id')
    .single();
  if (error) { console.warn('[user_sessions] no se pudo registrar ingreso:', error.message); return null; }
  return data?.id ?? null;
};

const fsSessionHeartbeat = async (sessionId, seccion) => {
  if (!sessionId) return;
  const { error } = await supabaseSession
    .from(COL_SESSIONS)
    // logout_at se limpia aquí a propósito: si estamos mandando un latido,
    // la sesión está viva por definición. Sin esto, un refresh de página
    // dispara el beacon de "salida" (beforeunload/pagehide) que marca
    // logout_at, y como el latido solo tocaba last_seen_at, la fila se
    // quedaba marcada como "Salió" para siempre aunque el usuario siguiera
    // activo — por eso el admin no se veía a sí mismo conectado.
    .update({ last_seen_at: new Date().toISOString(), seccion: seccion || null, logout_at: null })
    .eq('id', sessionId);
  if (error) console.warn('[user_sessions] error en latido:', error.message);
};

const fsSessionEnd = async (sessionId) => {
  if (!sessionId) return;
  const { error } = await supabaseSession
    .from(COL_SESSIONS)
    .update({ logout_at: new Date().toISOString(), seccion: null })
    .eq('id', sessionId);
  if (error) console.warn('[user_sessions] error al cerrar sesión:', error.message);
};

// Trae las sesiones recientes (para el panel de admin: quién está en línea
// y el historial de ingresos/salidas). `limit` acota el historial mostrado.
const fsGetSessions = async (limit = 200) => {
  const { data, error } = await supabaseSession
    .from(COL_SESSIONS)
    .select('*')
...
function ToastProvider({ children }) {
  ...
  const showToast = useCallback((msg, type = 'info', duration = 3400) => {
...
const STYLES = `
:root{
  --bd:#1A3F6F;--bm:#0060A0;--bl:#E8F2FA;--gold:#D4A800;--gold-d:#8A6B00;--gl:#FDF6DC;
  --g1:#F5F7FA;--g2:#E8ECF0;--g3:#CFD8DC;--g5:#78909C;--g7:#37474F;--g9:#1A2530;
  --red:#C62828;--grn:#2E7D32;--org:#D84315;
  /* ── depth tokens (refinado: elevación sutil, sin efecto "botón de caramelo") ── */
  --sh-btn:0 1px 2px rgba(16,32,56,.14),0 1px 0 rgba(255,255,255,.06) inset;
  --sh-btn-sm:0 1px 2px rgba(16,32,56,.12);
  --sh-hover:0 3px 8px rgba(16,32,56,.18);
  --sh-active:inset 0 1px 3px rgba(0,0,0,.18);
  --sh-card:0 1px 3px rgba(16,32,56,.06),0 1px 2px rgba(16,32,56,.04);
  --sh-card-hover:0 6px 16px rgba(16,32,56,.09),0 1px 3px rgba(16,32,56,.05);
  --sh-modal:0 16px 48px rgba(16,32,56,.28),0 0 0 1px rgba(255,255,255,.08);
  --sh-header:0 1px 0 rgba(255,255,255,.05) inset,0 2px 10px rgba(10,22,40,.18);
...
══════════════════════════════════ */
.ac-header{
  background:linear-gradient(115deg,var(--bd) 0%,#0f2c52 100%);
  padding:0 20px;border-bottom:2px solid var(--gold);
...
.ac-header::after{content:none}
.ac-hl{display:flex;align-items:center;gap:14px}
.ac-hdiv{width:1px;height:28px;background:rgba(255,255,255,.18)}
.ac-htitle .s1{font-size:0.49rem;color:rgba(255,255,255,.55);text-transform:uppercase;letter-spacing:1.4px;font-weight:600;display:block}
.ac-htitle .s2{font-size:0.79rem;color:#fff;font-weight:600;display:block;letter-spacing:.1px}
.ac-badge{
  background:rgba(255,255,255,.1);color:#fff;font-size:0.61rem;font-weight:600;
...
══════════════════════════════════ */
.btn{
  padding:8px 16px;border:1px solid transparent;border-radius:7px;font-size:0.67rem;font-weight:600;
  cursor:pointer;transition:background-color .12s ease,border-color .12s ease,box-shadow .12s ease;white-space:nowrap;
...
/* Primary — azul de marca, plano con acento sutil */
.btn-p{
  background:var(--bm);color:#fff;
  box-shadow:var(--sh-btn);
}
.btn-p:hover{background:#0a6cad;box-shadow:var(--sh-hover)}
.btn-p:active{background:#00568f;box-shadow:var(--sh-active)}

/* Gold / ámbar — reservado para acciones destacadas */
.btn-g{
  background:var(--gold);color:var(--bd);
  box-shadow:var(--sh-btn);
}
.btn-g:hover{background:#c29900;box-shadow:var(--sh-hover)}
.btn-g:active{background:#b08e00;box-shadow:var(--sh-active)}
...
.btn-c:hover{background:var(--glass-hover)}
.btn-c:active{background:rgba(255,255,255,.2)}
...
.btn-r:hover{background:#ab2020;box-shadow:var(--sh-hover)}
.btn-r:active{background:#8f1c1c;box-shadow:var(--sh-active)}

/* Outline */
.btn-clear{
  background:var(--red,#c62828);color:#fff;border:1px solid var(--red,#c62828);
  border-radius:8px;font-weight:700;box-shadow:0 2px 6px rgba(198,40,40,.35);
  flex-shrink:0;
}
.btn-clear:hover{background:#ab2020;border-color:#ab2020}
.btn-clear:active{background:#8f1b1b}
.btn-o{
  background:#fff;
  border:1.5px solid var(--g3);color:var(--bm);
  box-shadow:none;
}
.btn-o:hover{background:var(--bl);border-color:var(--bm)}
.btn-o:active{background:#dcedfa}
...
.btn-dark:hover{background:#153356;box-shadow:var(--sh-hover)}
.btn-dark:active{background:#0f2745;box-shadow:var(--sh-active)}
...
.btn-org:hover{background:#b8390f;box-shadow:var(--sh-hover)}
.btn-org:active{background:#9c300c;box-shadow:var(--sh-active)}
...
.btn-slate:hover{background:#455a64;box-shadow:var(--sh-hover)}
.btn-slate:active{background:#37474f;box-shadow:var(--sh-active)}
...
.btn-edit{
  padding:4px 10px;
  background:var(--bm);
  color:#fff;border:none;border-radius:6px;font-size:0.57rem;cursor:pointer;font-weight:600;
  box-shadow:var(--sh-btn-sm);
  display:inline-flex;align-items:center;justify-content:center;line-height:1;
  transition:background-color .12s ease,box-shadow .12s ease;touch-action:manipulation;
}
.btn-edit svg{display:block}
.btn-edit:hover{background:#0a6cad;box-shadow:var(--sh-hover)}
.btn-edit:active{background:#00568f;box-shadow:var(--sh-active)}

.btn-del{
  padding:4px 10px;
  background:#fff;
  color:var(--red);
  border:1px solid rgba(198,40,40,.35);border-radius:6px;font-size:0.57rem;cursor:pointer;font-weight:600;margin-left:4px;
  box-shadow:none;
  display:inline-flex;align-items:center;justify-content:center;line-height:1;
  transition:background-color .12s ease,color .12s ease;touch-action:manipulation;
}
.btn-del svg{display:block}
.btn-del:hover{background:var(--red);color:#fff}
.btn-del:active{background:#ab2020}
...
.cr-pill{
  ...
  background:var(--bd);
  padding:3px 6px 3px 12px;border-radius:6px;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;
  letter-spacing:.03em;line-height:1.4;
}
.cr-pill .ac-mark{background:rgba(255,255,255,.92);color:var(--bd);outline:none}
.btn-copy{
  padding:3px 7px;
  background:rgba(255,255,255,.22);
  border:1px solid rgba(255,255,255,.4);border-radius:4px;font-size:0.62rem;cursor:pointer;color:#fff;
  display:inline-flex;align-items:center;justify-content:center;
  transition:background-color .12s ease;line-height:1;
}
.btn-copy svg{display:block;width:13px;height:13px}
.btn-copy:hover{background:rgba(255,255,255,.36)}
.btn-copy:active{background:rgba(255,255,255,.46)}
...
.ac-sp.has-clasi-img{overflow:visible}
.ac-fl{display:flex;flex-direction:column;gap:3px;position:relative;z-index:1}
.ac-fl label{font-size:0.54rem;color:var(--bm);font-weight:700;text-transform:uppercase;letter-spacing:.7px;display:flex;align-items:center;gap:5px}
.ac-fl label svg{flex-shrink:0}
...
/* Panel de ícono — marca de agua decorativa que flota a la derecha,
   ...
   genera filas/huecos vacíos. Sin etiqueta de texto: solo el ícono. */
.ac-clasi-icon-panel{
...
.ac-clasi-icon-panel img.ac-clasi-img{
  opacity:.1 !important ;position:absolute;right:0;top:50%;transform:translateY(calc(-50% + 70px));
  width:480px;height:auto;max-width:none;object-fit:contain;z-index:0;
  /* Difumina el borde/contorno propio del PNG (no transparente) para que se
     funda con el fondo en vez de verse como un recuadro gris */
...
.ac-sr-mid input[type="text"]{box-sizing:border-box;width:100%}
...
thead th{
  ...
  font-size:0.56rem;font-weight:600;text-transform:uppercase;letter-spacing:.6px;
  position:sticky;top:0;z-index:10;white-space:nowrap;
  border-right:1px solid rgba(255,255,255,.08);cursor:pointer;user-select:none;transition:background-color .12s;
}
...
.dec-pg-toggle.active{
  ...
  border-color:var(--bm)!important;color:#fff!important;font-weight:600!important;
}
tbody tr{transition:background-color .1s;background:#fff}
...
.cmo{color:var(--g5);font-style:italic;white-space:normal;font-weight:500;word-break:break-word;line-height:1.3}
...
.pb:active{background:#dcedfa}
.pb.active{
  background:var(--bm);
  border-color:var(--bm);color:#fff;font-weight:600;
}
...
.mh.danger{background:var(--red)}
.mh h2{font-size:0.82rem;color:#fff;font-weight:600;line-height:1}
.mh h2 svg{display:block}
.mx{
...
/* ── Detalle del Registro — panel rediseñado ── */
.dtl-md{max-width:1024px;width:96vw}
.dtl-hleft{display:flex;align-items:center;gap:12px;min-width:0;flex:1}
.dtl-hicon{width:42px;height:42px;border-radius:50%;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff}
.dtl-htitles{display:flex;flex-direction:column;gap:4px;min-width:0}
.dtl-htitles h2{font-size:1.05rem;color:#fff;font-weight:700;line-height:1.1}
.dtl-hrow2{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.dtl-hcode{font-size:1rem;color:#fff;font-weight:800;font-family:'SF Mono','Fira Code','Consolas',monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:420px}
.dtl-verif-wrap{display:flex;align-items:center;gap:10px;flex-shrink:0;margin:0 14px}
@keyframes verif-pulse{
  0%{r:12;opacity:1}
  50%{r:13.4;opacity:.75}
  100%{r:12;opacity:1}
}
.verif-pulse{transform-origin:13px 13px;animation:verif-pulse 1.6s ease-in-out infinite}
.dtl-verif-bubble{position:relative;background:#0071c2;color:#fff;font-size:0.66rem;font-weight:700;padding:6px 13px;border-radius:12px;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,.18)}
.dtl-verif-bubble::before{content:'';position:absolute;left:-6px;top:50%;transform:translateY(-50%);border-width:6px 7px 6px 0;border-style:solid;border-color:transparent #0071c2 transparent transparent}
.dtl-verif-bubble.gray{background:#546E7A}
.dtl-verif-bubble.gray::before{border-color:transparent #546E7A transparent transparent}
.dtl-body{padding:14px 18px 4px;display:flex;flex-direction:column;gap:9px}
.dtl-info-grid{display:grid;grid-template-columns:1fr;gap:10px}
.dtl-item{display:flex;align-items:center;gap:10px;background:var(--g1);border:1px solid var(--g2);border-radius:9px;padding:8px 12px}
.dtl-item.mini{padding:5px 10px;gap:8px}
.dtl-item-ic{width:34px;height:34px;border-radius:10px;background:var(--bl);color:var(--bm);display:flex;align-items:center;justify-content:center;flex-shrink:0}
.dtl-item-ic svg{width:27px;height:27px}
.dtl-item-ic.mini{width:24px;height:24px;border-radius:8px}
.dtl-item-ic.mini svg{width:21px;height:21px}
.dtl-item-body{min-width:0;flex:1}
.dtl-item-lbl{font-size:0.55rem;font-weight:700;color:var(--bm);text-transform:uppercase;letter-spacing:.6px;margin-bottom:1px}
.dtl-item-lbl.inline{margin-bottom:0;white-space:nowrap;flex-shrink:0}
.dtl-item-val{font-size:0.86rem;font-weight:700;color:var(--g9);word-break:break-word;line-height:1.25}
.dtl-item-val.empty{color:var(--g5);font-weight:500;font-style:italic;font-size:0.78rem}
.dtl-item-val.inline{font-size:0.74rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0}
.dtl-codes-footer{border-top:1px solid var(--g2);padding-top:9px;margin-top:2px}
.dtl-codes-footer-title{display:flex;align-items:center;gap:7px;font-size:0.6rem;font-weight:700;color:var(--bm);text-transform:uppercase;letter-spacing:.6px;margin-bottom:7px}
.dtl-codes-chips{display:flex;flex-wrap:wrap;gap:8px}
.dtl-code-chip{display:flex;align-items:center;gap:7px;background:linear-gradient(135deg,var(--bl),#dcecfa);border:1px solid #BEDCF5;border-radius:20px;padding:4px 6px 4px 6px}
.dtl-code-chip .num{width:20px;height:20px;border-radius:50%;background:var(--bm);color:#fff;font-size:0.58rem;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.dtl-code-chip .lbl{font-size:0.58rem;font-weight:800;color:var(--bm);text-transform:uppercase;letter-spacing:.4px;white-space:nowrap;padding-left:6px}
...
.dtl-code-chip button svg{width:24px;height:24px}
.dtl-code-chip button:hover{background:#fff}
.dtl-body-split{display:grid;grid-template-columns:1fr 430px;gap:20px;align-items:start}
...
.dtl-code-chip-aside{width:100%;box-sizing:border-box;padding:7px 10px;gap:9px;border-radius:10px}
@media(max-width:680px){.dtl-info-grid{grid-template-columns:1fr}.dtl-hcode{max-width:160px}.dtl-verif-wrap{margin:0 6px}.dtl-body-split{grid-template-columns:1fr}.dtl-codes-aside{border-left:none;border-top:1px solid var(--g2);padding-left:0;padding-top:9px}}
...
.toast{
  ...
  z-index:10050;display:none;color:#fff;max-width:360px;
...
.hop-add{color:var(--grn)}.hop-edit{color:var(--bm)}.hop-del{color:var(--red)}.hop-imp{color:var(--org)}.hop-rep{color:#C62828}
...
/* ── Novedades — píldora chica, alineada con la fila de Clasificación/
   Subclasificación, dentro del panel de filtros (no flotante) ── */
.novf-inline-wrap{
  position:relative;display:flex;flex-direction:column;align-items:flex-end;
  justify-content:center;z-index:2;min-width:0;
}
.novf-pill-holder{position:relative;display:inline-flex;max-width:100%}
.novf-pill{
  display:inline-flex;align-items:center;gap:8px;cursor:pointer;user-select:none;
  background:linear-gradient(135deg,var(--bd) 0%,#0d52a8 60%,var(--bm) 100%);color:#fff;
  border-radius:999px;padding:6px 12px;box-shadow:0 2px 8px rgba(0,0,0,.18);
...
.novf-cat-icon{display:flex;align-items:center;justify-content:center;flex-shrink:0;opacity:.95;width:24px;height:24px;background:#fff;border:1px solid #ccc;border-radius:50%;box-sizing:border-box}
...
.novf-chev-btn{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:5px;flex-shrink:0;transition:background-color .12s ease}
.novf-chev-btn:hover{background:rgba(255,255,255,.18)}
.novf-chev{display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:1.75rem;font-weight:800;line-height:1;transition:transform .2s ease;flex-shrink:0}
.novf-chev.open{transform:rotate(90deg)}
.novf-close-float{
  position:absolute; top:-7px; left:-7px; z-index:5;
  width:18px; height:18px; padding:0; border-radius:50%;
  background:#fff; border:1.5px solid var(--red,#e33); color:var(--red,#e33);
  display:flex; align-items:center; justify-content:center; cursor:pointer;
  box-shadow:0 1px 4px rgba(0,0,0,.3); transition:background-color .12s ease, color .12s ease;
}
.novf-close-float:hover{background:var(--red,#e33); color:#fff}
@keyframes novf-bell-ring{
...
.novf-bell-ring{transform-origin:50% 0;animation:novf-bell-ring .6s ease-in-out 2}
@keyframes novf-pill-rise{
  0%{opacity:0;transform:translateY(10px)}
  100%{opacity:1;transform:translateY(0)}
}
.novf-pill-rise{animation:novf-pill-rise .35s ease-out}
.novf-dropdown{
  position:absolute;top:calc(100% + 8px);right:0;z-index:30;
  display:flex;flex-direction:column;align-items:flex-end;gap:8px;
}
.novf-card{
  background:#fff;border:1px solid var(--gold-d);border-radius:12px;
  box-shadow:0 10px 26px rgba(0,0,0,.18);padding:2px 14px;width:300px;max-width:88vw;
  cursor:pointer;overflow:hidden;
}
.novf-tagline{
  display:block;width:100%;box-sizing:border-box;
  margin:0 0 4px;padding:7px 0 0;
  font-size:0.54rem;font-weight:800;letter-spacing:.5px;color:var(--gold-d);text-align:left;
  background:none;
}
.novf-row{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--g2)}
.novf-row-last{border-bottom:none}
.novf-row:last-child{border-bottom:none}
.novf-ic{
  width:24px;height:24px;border-radius:50%;background:var(--gl);color:var(--gold-d);
  display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:0.68rem;font-weight:800;
}
.novf-row div{color:var(--bd);font-size:0.66rem}
.novf-row div b{color:var(--bd)}
.novf-gold{color:var(--gold-d)!important}
.novf-small{font-size:0.58rem;color:var(--g5);font-weight:500;margin-top:1px}
.novf-pager{
  display:flex;align-items:center;gap:8px;
  background:#fff;border-radius:999px;padding:3px 10px;box-shadow:0 4px 12px rgba(0,0,0,.15);
  font-size:0.6rem;color:var(--bd);font-weight:600;
}
.novf-pager button{background:none;border:none;cursor:pointer;font-size:0.85rem;color:var(--bd);
  padding:0 3px;line-height:1}
.novf-pager button:disabled{opacity:.3;cursor:default}
@media (max-width:900px){
  .novf-inline-wrap{display:none} /* en pantallas chicas se prioriza el filtro; la fila 2 ya muestra Marca/Modelo */
}

/* Progress overlay — pantalla emergente a pantalla completa */
.ac-progress-overlay{
...
.ac-progress-card{
  background:transparent;
  border:none;border-radius:0;
  padding:0;width:auto;height:auto;max-width:none;max-height:none;min-width:0;
...
.ac-progress-title{
  font-size:1.05rem;font-weight:700;color:#fff;letter-spacing:.1px;
  margin-top:30px;display:flex;align-items:center;gap:8px;
}
.ac-progress-title .dot{display:none}
.ac-progress-spin-wrap{position:relative;width:140px;height:140px}
.ac-progress-spin{
  width:140px;height:140px;
  border-radius:50%;
  background:conic-gradient(from 0deg, var(--bm) 0%, var(--gold) 45%, rgba(255,255,255,0) 75%, rgba(255,255,255,0) 100%);
  ...
  /* transform-origin explícito + box-sizing evitan que el aro "cabecee"
     en vez de girar limpio (el mask + el conic-gradient sin origen fijo
     dejaba el pivote desplazado en algunos navegadores) */
  transform-origin:50% 50%;
  box-sizing:border-box;
  /* Gira en sentido antihorario a propósito: el conic-gradient define la
     "cola" (parte que se desvanece) en sentido horario respecto a la cabeza
     sólida. Si el anillo rotara en horario, la cola quedaría delante de la
     cabeza en vez de detrás (se veía "al revés"). Con rotación antihoraria
     la cabeza queda al frente y la cola se desvanece correctamente detrás. */
  animation:spin-rev .95s linear infinite;
  will-change:transform;
}
@keyframes spin-rev{to{transform:rotate(-360deg)}}
.ac-progress-label{font-size:0.74rem;color:rgba(255,255,255,.75);font-weight:500;
  display:block;margin-top:10px;max-width:420px;white-space:normal;text-align:center;line-height:1.4}
...
@media(max-width:768px){
  ...
  .ac-clasi-icon-panel svg{width:100px;height:100px}
  .ac-clasi-icon-panel img.ac-clasi-img{width:220px}
...
.dec-top{display:flex;align-items:center;gap:8px;padding:8px 12px;flex-wrap:wrap;
  background:var(--bd);border-bottom:2px solid var(--gold)}
...
.dec-btn{
  ...
  cursor:pointer;letter-spacing:.05em;transition:background-color .12s ease;white-space:nowrap;
}
.dec-btn:hover{background:#c29900}
.dec-btn:active{background:#b08e00}
...
const ModalEdit = ({ record, onSave, onClose }) => {
  ...
  const labels = ['Marca *','Modelo *','Modelo Original','Período *',
    ...
    'Descripción Estándar','Clasificación','Subclasificación','Especificación'];
    ...
    if (addingNew && addingNew.field === i) return (
      <div style={{display:'flex',gap:5}}>
        ...
        <button type="button" onClick={confirmAddNew}
          style={{padding:'0 12px',background:'var(--grn)',color:'#fff',border:'none',borderRadius:6,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center'}}><Icon.Check/></button>
        <button type="button" onClick={()=>setAddingNew(null)}
          style={{padding:'0 10px',background:'var(--g3)',color:'var(--g7)',border:'none',borderRadius:6,cursor:'pointer',display:'inline-flex',alignItems:'center'}}><Icon.X/></button>
      </div>
    );
    if (i===0) return (
  ...
  return (
    <div className="mo show">
      <div className="md">
        <div className="mh">
          <h2 style={{display:'flex',alignItems:'center',gap:8}}>{isNew ? <><Icon.Plus/>Nuevo Registro</> : <><Icon.Pencil/>Editar Registro</>}</h2>
        ...
        <div className="mf">
          <button className="btn btn-o" onClick={onClose} disabled={saving}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cancelar</button>
          <button className="btn btn-p" onClick={handleSave} disabled={saving}>
            {saving ? <><span className="spin" style={{width:14,height:14,borderWidth:2}}/>Guardando…</> : <><Icon.Check style={{marginRight:6,verticalAlign:-2}}/>Guardar</>}
          </button>
        </div>
      </div>
    </div>
...
const ModalDelete = ({ record, onConfirm, onClose }) => {
  ...
  return (
    <div className="mo show">
      <div className="md sm">
        <div className="mh danger"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Trash/>Eliminar Registro</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="mb">
        ...
        <div className="mf">
          <button className="btn btn-o" onClick={onClose}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cancelar</button>
          <button className="btn btn-r" onClick={onConfirm}><Icon.Trash style={{marginRight:6,verticalAlign:-2}}/>Eliminar</button>
        </div>
      </div>
    </div>
  );
...
//  MODAL — DETALLE
// ============================================================
const REPORTE_TIPOS = ['Código incorrecto', 'Dato incorrecto (marca/modelo/período)', 'Clasificación incorrecta', 'Otro'];

// ── Insignia de verificación (estilo Instagram) ──
// Círculo azul fijo; dentro, dos palomitas. Cada una se pone blanca (llena)
// según cuántas confirmaciones distintas lleva el código: 0 = ambas gris
// clarito, 1 = una blanca, 2 (umbral) = ambas blancas = verificado.
// Un clic registra la confirmación del usuario actual (una sola vez).
const VerifBadge = ({ count, threshold, yaVerifique, loading, onClick, size = 26 }) => {
  const check1 = count >= 1;
  const check2 = count >= threshold;
  const dim = 'rgba(255,255,255,0.4)';
  const bright = '#fff';
  const title = check2
    ? 'Verificado por la comunidad'
    : yaVerifique
      ? 'Ya confirmaste — falta al menos 1 confirmación más de otro usuario'
      : `Si esta correcto Verifícalo (${count}/${threshold})`;
  const handleClick = () => { if (!loading && !yaVerifique && !check2) onClick?.(); };
  return (
    <svg
      width={size} height={size} viewBox="0 0 26 26"
      onClick={handleClick}
      style={{cursor:(loading||yaVerifique||check2)?'default':'pointer',flexShrink:0,opacity:loading?0.6:1}}
    >
      <title>{title}</title>
      <circle cx="13" cy="13" r="12" fill="#0060A0" className={!check2 && !loading ? 'verif-pulse' : ''}/>
      {/* palomita 1 — sombra ligera para distinguirla de la 2 cuando ambas están llenas */}
      <path d="M6.9 14.6l2.6 2.6 3.4-5.2" fill="none" stroke={check1?bright:dim} strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter" style={{filter:'drop-shadow(0 1px 1px rgba(0,0,0,.55))'}}/>
      {/* palomita 2 */}
      <path d="M10.1 14.6l2.6 2.6 6.4-8.4" fill="none" stroke={check2?bright:dim} strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter" style={{filter:'drop-shadow(0 1px 1px rgba(0,0,0,.4))'}}/>
    </svg>
  );
};

// ── Insignia mini de verificado (misma forma del VerifBadge ya completo,
//    sin interacción) — para usar junto a la píldora de código en la tabla ──
const MiniVerifBadge = ({ size = 18, title = `Verificado por ${VERIF_THRESHOLD}+ usuarios` }) => (
  <svg width={size} height={size} viewBox="0 0 26 26" title={title} style={{flexShrink:0}}>
    <circle cx="13" cy="13" r="12" fill="#0060A0"/>
    <path d="M6.9 14.6l2.6 2.6 3.4-5.2" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter" style={{filter:'drop-shadow(0 1px 1px rgba(0,0,0,.55))'}}/>
    <path d="M10.1 14.6l2.6 2.6 6.4-8.4" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter" style={{filter:'drop-shadow(0 1px 1px rgba(0,0,0,.4))'}}/>
  </svg>
);

const ModalDetail = ({ record, onClose, onEdit, onReported, onVerified }) => {
  const toast = useToast();
  const { user, isAdmin } = useAuth();
  const [reporting, setReporting]   = useState(false);
  const [rTipo, setRTipo]           = useState(REPORTE_TIPOS[0]);
  ...
  useEffect(() => {
    let mounted = true;
    setVerifs([]); setVerifsLoaded(false);
    if (record?._id) {
      fsGetVerificaciones(record._id)
        .then(v => { if (mounted) { setVerifs(v); setVerifsLoaded(true); } })
        .catch(() => { if (mounted) setVerifsLoaded(true); });
    }
  ...
  const detailLabels = {0:'Marca',1:'Modelo',2:'Modelo Original',3:'Período',
    ...
    10:'Desc. Estándar',11:'Clasificación',12:'Subclasificación',13:'Especificación'};
  // Mismo orden que la tabla; 10 (Desc. Estándar) oculto hasta segunda orden
  const detailOrder = [0,1,13,3,11,12,4,5,6,7,8,9,2];

  const yaVerifique = verifs.some(v => v.usuario === user?.email);

  const confirmarVerificacion = async () => {
    if (!record._id || !user?.email) return;
    setVerifSaving(true);
    try {
      await fsAddVerificacion(record._id, user.email);
      const nuevos = [...verifs, { usuario: user.email }];
      setVerifs(nuevos);
      const quedaVerificado = new Set(nuevos.map(v=>v.usuario)).size >= VERIF_THRESHOLD;
      if (quedaVerificado) onVerified?.(record._id, true);
      toast(quedaVerificado ? 'Confirmado — este código ya quedó verificado.' : 'Confirmación registrada. Falta al menos 1 confirmación más.', 'success');
    } catch(e) {
  ...
  const enviarReporte = async () => {
    if (!rComentario.trim()) { toast('Describe brevemente el problema antes de enviar.', 'error'); return; }
    setRSaving(true);
    try {
      const { fecha, hora } = nowDT();
      const nuevoReporte = {
        fecha, hora,
        codigo: record.fields[4] || '',
        marca: record.fields[0] || '',
        modelo: record.fields[1] || '',
        clasi: record.fields[11] || '',
        sub: record.fields[12] || '',
        registro_id: record._id || null,
        tipo: rTipo,
        comentario: rComentario.trim(),
        usuario: user?.email || 'anónimo',
        estado: 'abierto',
      };
      await fsAddReporte(nuevoReporte);
      // Refleja el reporte en el historial de inmediato (sin esperar recarga)
      onReported?.({ ...nuevoReporte, id: `local-${Date.now()}` });
      // El trigger en Supabase revoca la verificación de este código al
      // reportarlo — reflejamos eso de inmediato en la UI también.
      if (record.verificado) { onVerified?.(record._id, false); setVerifs([]); }
      toast('Reporte enviado. Un administrador lo revisará pronto.', 'success');
      setReporting(false); setRComentario(''); setRTipo(REPORTE_TIPOS[0]);
    } catch (e) {
      toast('Error al enviar el reporte: ' + e.message, 'error');
    } finally {
      setRSaving(false);
    }
  };

  // ── Datos para el layout rediseñado ──
  const f = record.fields;
  const estaVerificado = new Set(verifs.map(v=>v.usuario)).size >= VERIF_THRESHOLD;

  const copyVal = (val) => { navigator.clipboard?.writeText(val); toast('Código copiado', 'info'); };

  // El código de repuesto principal (combinado) encabeza la lista, seguido
  // de código_1..código_5 — se numeran del 1 en adelante solo los que existen.
  const CODIGO_LABELS = {4:'Código Repuesto',5:'Código 1',6:'Código 2',7:'Código 3',8:'Código 4',9:'Código 5'};
  const codigosLista = [4,5,6,7,8,9]
    .map(i=>({ label: CODIGO_LABELS[i], val: f[i] }))
    .filter(c=>c.val && c.val.trim());
  // El combinado ("Código Repuesto") es redundante y no se muestra cuando:
  //  a) hay 2 o más códigos individuales (ya se ve el split completo), o
  //  b) hay solo 1 código individual y su valor es igual al combinado
  //     (no hubo split real, es el mismo código repetido).
  const combinado = codigosLista.find(c=>c.label==='Código Repuesto');
  const individuales = codigosLista.filter(c=>c.label!=='Código Repuesto');
  const combinadoEsRedundante = combinado && (
    individuales.length >= 2 ||
    (individuales.length === 1 && individuales[0].val.trim() === combinado.val.trim())
  );
  const codigosListaVisible = combinadoEsRedundante ? individuales : codigosLista;

  const leftItems = [
    { icon:<Icon.Tag/>,     lbl:'Marca',            val:f[0] },
    { icon:<Icon.Car/>,     lbl:'Modelo',           val:f[1] },
    { icon:<Icon.Calendar/>,lbl:'Período',          val:f[3] },
    { icon:<Icon.Layers/>,  lbl:'Clasificación',    val:f[11] },
    { icon:<Icon.Network/>, lbl:'Subclasificación', val:f[12] },
  ];

  return (
    <div className="mo show">
      <div className="md dtl-md">
        <div className="mh">
          <div className="dtl-hleft">
            <div className="dtl-hicon"><Icon.Doc/></div>
            <div className="dtl-htitles">
              <h2>Detalle del Registro</h2>
              <div className="dtl-hrow2">
                {f[4] && <span className="dtl-hcode" title={f[4]}>{f[4]}</span>}
              </div>
            </div>
            <div className="dtl-verif-wrap">
              <VerifBadge
                count={new Set(verifs.map(v=>v.usuario)).size}
                threshold={VERIF_THRESHOLD}
                yaVerifique={yaVerifique}
                loading={!verifsLoaded || verifSaving}
                onClick={confirmarVerificacion}
                size={54}
              />
              <span className={`dtl-verif-bubble${estaVerificado?'':' gray'}`}>
                {estaVerificado ? 'Verificado' : 'Verifica'}
              </span>
            </div>
          </div>
          <button className="mx" onClick={onClose}>×</button>
        ...
        <div className="dtl-body dtl-body-split">
          <div className="dtl-main-col">
          {/* Datos del vehículo — grid 2 columnas, aprovecha el ancho del panel */}
          <div className="dtl-info-grid">
            {leftItems.map(it => (
              <div key={it.lbl} className="dtl-item">
                <div className="dtl-item-ic">{it.icon}</div>
                <div className="dtl-item-body">
                  <div className="dtl-item-lbl">{it.lbl}</div>
                  <div className={`dtl-item-val${it.val?'':' empty'}`}>{it.val || '—'}</div>
                </div>
              </div>
            ))}
          </div>

          {/* Campos adicionales — ancho completo, muy compactos (una línea) */}
          {[
            { icon:<Icon.Doc/>,  lbl:'Descripción',      val:f[10] },
            { icon:<Icon.Doc/>,  lbl:'Modelo Original',  val:f[2] },
            { icon:<Icon.Link/>, lbl:'Equivalencia',     val:'' },
          ].map(it => (
            <div key={it.lbl} className="dtl-item mini" style={{height:14,minHeight:14,overflow:'hidden'}}>
              <div className="dtl-item-ic mini">{it.icon}</div>
              <span className="dtl-item-lbl inline">{it.lbl}</span>
              <span className={`dtl-item-val${it.val?'':' empty'} inline`}>{it.val || '—'}</span>
            </div>
          ))}
          </div>

          {/* Columna derecha — listado de Código de Repuesto (1,2,3,4,5) */}
          <div className="dtl-codes-footer dtl-codes-aside">
            <div className="dtl-codes-footer-title">
              <Icon.Doc style={{width:13,height:13}}/>Código de Repuesto
            </div>
            {codigosListaVisible.length === 0 ? (
              <div style={{fontSize:'0.68rem',color:'var(--g5)',fontStyle:'italic'}}>Sin códigos registrados</div>
            ) : (
              <div className="dtl-codes-chips dtl-codes-chips-aside">
                {codigosListaVisible.map((c,i) => (
                  <div key={i+c.val} className="dtl-code-chip dtl-code-chip-aside">
                    <span className="lbl">{c.label}</span>
                    <span className="val">{c.val}</span>
                    <button onClick={()=>copyVal(c.val)} title="Copiar"><Icon.Copy/></button>
                  </div>
                ))}
              </div>
            )}
          ...
          {reporting && (
            <div style={{padding:12,borderRadius:8,background:'var(--g1)',border:'1px solid var(--g3)'}}>
              <div style={{fontSize:'0.68rem',fontWeight:700,color:'var(--bd)',marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
                <Icon.Alert style={{color:'var(--red)'}}/>Reportar este registro
              </div>
              <select value={rTipo} onChange={e=>setRTipo(e.target.value)}
                style={{width:'100%',marginBottom:8,border:'1.5px solid var(--g3)',borderRadius:6,padding:'6px 8px',fontSize:'0.68rem'}}>
                {REPORTE_TIPOS.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
              <textarea value={rComentario} onChange={e=>setRComentario(e.target.value)}
                placeholder="Describe qué está mal (ej. el código no corresponde al modelo, la marca es incorrecta, etc.)"
                rows={3}
                style={{width:'100%',border:'1.5px solid var(--g3)',borderRadius:6,padding:'6px 8px',fontSize:'0.68rem',resize:'vertical',fontFamily:'inherit'}}/>
              <div style={{display:'flex',gap:8,marginTop:8,justifyContent:'flex-end'}}>
                <button className="btn btn-o btn-sm" onClick={()=>{setReporting(false);setRComentario('');}} disabled={rSaving}>Cancelar</button>
                <button className="btn btn-p btn-sm" onClick={enviarReporte} disabled={rSaving}>
                  {rSaving ? 'Enviando…' : 'Enviar reporte'}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="mf">
          <button className="btn btn-o" onClick={onClose}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cerrar</button>
          {!reporting && <button className="btn btn-o" onClick={()=>setReporting(true)}
            style={{color:'var(--red)',borderColor:'var(--red)'}}>
            <Icon.Alert style={{marginRight:6,verticalAlign:-2}}/>Reportar
          </button>}
          {isAdmin && <button className="btn btn-p" onClick={()=>{onClose();onEdit(record);}}><Icon.Pencil style={{marginRight:6,verticalAlign:-2}}/>Editar</button>}
        </div>
      </div>
    </div>
  );
...
const ModalImport = ({ onClose, onImport, defaultTarget='produccion' }) => {
  ...
  const doImport = async (mode) => {
    ...
    onClose();
    await onImport(records, mode, target, { motivo: motivo.trim(), fileName });
  };

  return (
    <div className="mo show">
      <div className="md">
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Folder/>Cargar Base de Datos</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="mb">
          ...
          >
            ...
            {loading
              ...
              : <><div className="icon"><Icon.Folder style={{width:36,height:36}}/></div>
          ...
          <div className="wb">
            ...
            <strong> Agregar</strong> añade a la aplicación sin borrar nada.
            <br/>Destino seleccionado: <strong>{target==='contingencia'?'Contingencia':'Producción'}</strong>
          </div>

          <div style={{marginTop:12}}>
            <label style={{fontSize:'0.67rem',fontWeight:700,color:'var(--g7)',display:'block',marginBottom:6}}>
              Motivo de la carga
              {fileName && <span style={{fontWeight:400,color:'var(--g5)',marginLeft:8,fontSize:'0.59rem'}}>
                Archivo de origen: <strong>{fileName}</strong>
              </span>}
            </label>
            <textarea
              value={motivo}
              onChange={e=>setMotivo(e.target.value)}
              placeholder="Ej. Actualización mensual de catálogo, corrección de códigos duplicados, etc."
              rows={2}
              style={{width:'100%',border:'1.5px solid var(--g3)',borderRadius:6,padding:'6px 8px',fontSize:'0.68rem',resize:'vertical',fontFamily:'inherit',boxSizing:'border-box'}}
            />
          </div>
        </div>
        <div className="mf">
          <button className="btn btn-o" onClick={onClose}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cancelar</button>
          <button className="btn btn-org" onClick={()=>doImport('replace')} disabled={!parsed||loading}><Icon.Refresh style={{marginRight:6,verticalAlign:-2}}/>Reemplazar</button>
          <button className="btn btn-p"   onClick={()=>doImport('append')}  disabled={!parsed||loading}><Icon.Plus style={{marginRight:6,verticalAlign:-2}}/>Agregar</button>
        </div>
      </div>
    </div>
  );
...
// ============================================================
const ModalCols = ({ visibleCols, colOrder, onChange, onReorder, onRestore, onClose }) => {
  ...
  return (
    <div className="mo show">
      <div className="md sm">
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Eye/>Columnas — Visibilidad y Orden</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="mb">
        ...
        <div className="mf">
          ...
          <button className="btn btn-o" onClick={()=>{
            setLocalOrder([...COL_DEFS_ORDER]);
            onRestore();
          }}><Icon.Refresh style={{marginRight:6,verticalAlign:-2}}/>Restaurar</button>
          <button className="btn btn-p" onClick={()=>{onReorder(localOrder);onClose();}}><Icon.Check style={{marginRight:6,verticalAlign:-2}}/>Personalizar</button>
        </div>
      </div>
    </div>
  );
...
const ModalReplace = ({ cols, records, onReplace, onClose }) => {
  ...
  return (
    <div className="mo show">
      <div className="md sm">
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Refresh/>Reemplazo Masivo</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="mb">
        ...
        <div className="mf">
          <button className="btn btn-o" onClick={onClose} disabled={running}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cerrar</button>
          <button className="btn btn-p" onClick={doReplace} disabled={running||!searchVal.trim()}>
            {running ? <><span className="spin" style={{width:13,height:13,borderWidth:2}}/>Procesando…</> : <><Icon.Refresh style={{marginRight:6,verticalAlign:-2}}/>Reemplazar</>}
          </button>
        </div>
      </div>
    </div>
...
const ModalHistory = ({ changelog, onClose }) => {
  ...
  const opIcon={AGREGAR:<Icon.Check/>,EDITAR:<Icon.Pencil/>,ELIMINAR:<Icon.Trash/>,IMPORTAR:<Icon.Folder/>,REPORTAR:<Icon.Alert/>};
  const opCls ={AGREGAR:'hop-add',EDITAR:'hop-edit',ELIMINAR:'hop-del',IMPORTAR:'hop-imp',REPORTAR:'hop-rep'};

  const exportCSV = () => {
    if (!changelog.length) return;
    const rows=[['#','Fecha','Hora','Usuario','Operación','Resumen','Campos Cambiados']];
    changelog.forEach(e=>{
      const cambios=(e.cambios||[]).map(c=>`${c.campo}: "${c.antes}"→"${c.despues}"`).join(' | ');
      rows.push([e.id,e.fecha,e.hora,e.usuario||'desconocido',e.op,e.resumen,cambios]);
    });
  ...
  return (
    <div className="mo show">
      <div className="md" style={{maxWidth:860,width:'95vw'}}>
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.History/>Historial de Cambios</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="hist-toolbar">
          <span style={{display:'inline-flex',alignItems:'center',color:'var(--bm)'}}>
            {filter ? opIcon[filter] : <Icon.History/>}
          </span>
          ...
          <button className="btn btn-dark btn-sm" style={{marginLeft:'auto'}} onClick={exportCSV}><Icon.Download style={{marginRight:6,verticalAlign:-2}}/>Exportar historial</button>
        </div>
            ...
            : list.map((e,idx)=>(
              <div key={idx} className="hlog-item">
                ...
                <div className="hlog-det">
                  ...
                  <div className="hlog-ip">{e.usuario||'desconocido'}</div>
                </div>
              </div>
            ))
          }
        </div>
        <div className="mf"><button className="btn btn-o" onClick={onClose}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Cerrar</button></div>
      </div>
    </div>
  );
};
...
// ============================================================
//  MODAL — SESIONES DE USUARIO (ingreso/salida + quién está conectado)
// ============================================================
const ModalSessions = ({ sessions, onClose }) => {
  const now = Date.now();
  const isOnline = s => !s.logout_at && (now - new Date(s.last_seen_at || s.login_at).getTime()) < SESSION_ONLINE_THRESHOLD_MS;
  const online = sessions.filter(isOnline);

  const fmt = iso => iso ? new Date(iso).toLocaleString('es-PA', { dateStyle:'short', timeStyle:'short' }) : '—';
  // Cuando no llegó logout_at (ej. el navegador se cerró sin dar tiempo al
  // beacon de salida) usamos el último latido como salida aproximada, para
  // no dejar el campo vacío en sesiones que claramente ya terminaron.
  const salidaTxt = s => {
    if (s.logout_at) return fmt(s.logout_at);
    if (isOnline(s)) return '—';
    return `${fmt(s.last_seen_at)} (aprox.)`;
  };
  const duracion = s => {
    const ini = new Date(s.login_at).getTime();
    const fin = s.logout_at ? new Date(s.logout_at).getTime()
      : (isOnline(s) ? now : new Date(s.last_seen_at || s.login_at).getTime());
  ...
  return (
    <div className="mo show">
      <div className="md" style={{maxWidth:820,width:'95vw'}}>
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Eye/>Usuarios</h2><button className="mx" onClick={onClose}>×</button></div>

        <div style={{padding:'12px 20px 0'}}>
          <strong style={{fontSize:'0.75rem'}}>Conectados ahora ({online.length})</strong>
          <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:8,marginBottom:14}}>
            {online.length===0 && <span style={{fontSize:'0.65rem',color:'var(--g5)'}}>Nadie conectado en este momento.</span>}
            {online.map(s=>(
              <span key={s.id} style={{display:'inline-flex',alignItems:'center',gap:6,background:'var(--bl)',border:'1px solid var(--g3)',borderRadius:14,padding:'4px 10px',fontSize:'0.65rem'}}>
                <span style={{width:8,height:8,borderRadius:'50%',background:'#2e7d32',display:'inline-block'}}/>
                {s.email}
                {s.seccion && <span style={{color:'var(--g5)'}}>· {s.seccion}</span>}
                {!s.seccion && <span style={{color:'var(--g5)'}}>· Sin búsqueda activa</span>}
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="mhist-wrap">
          <strong style={{fontSize:'0.75rem',padding:'0 20px',display:'block',marginBottom:6}}>Historial de ingresos</strong>
          {sessions.length===0
            ? <div className="hist-empty">Sin registros aún.</div>
            : sessions.map(s=>(
              <div key={s.id} className="hlog-item">
                <div className="hlog-dt">{fmt(s.login_at)}</div>
                <div className={`hlog-op ${isOnline(s)?'hop-add':'hop-edit'}`}>{isOnline(s)?'En línea':'Salió'}</div>
                <div className="hlog-det">
                  <div><strong>{s.email}</strong></div>
                  <span className="field-chg">
                    <em>Salida:</em> {salidaTxt(s)} · <em>Duración:</em> {duracion(s)}
                  </span>
                </div>
              </div>
            ))
...
// ============================================================
//  MODAL — REPORTES (ticket helpdesk de códigos/datos erróneos)
// ============================================================
const ModalReportes = ({ reportes, onClose, onUpdateEstado }) => {
  const [filter, setFilter] = useState('abierto');
  const [busyId, setBusyId] = useState(null);
  const list = filter === 'todos' ? reportes : reportes.filter(r => r.estado === filter);
  const abiertos = reportes.filter(r => r.estado === 'abierto').length;

  const toggle = async (r) => {
    setBusyId(r.id);
    await onUpdateEstado(r.id, r.estado === 'abierto' ? 'resuelto' : 'abierto');
    setBusyId(null);
  };
  ...
  return (
    <div className="mo show">
      <div className="md" style={{maxWidth:860,width:'95vw'}}>
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Alert/>Reportes de Usuarios{abiertos>0 && ` (${abiertos} pendiente${abiertos===1?'':'s'})`}</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="hist-toolbar">
          <span style={{display:'inline-flex',alignItems:'center',color:'var(--bm)'}}><Icon.Alert/></span>
          <select value={filter} onChange={e=>setFilter(e.target.value)} style={{border:'1.5px solid var(--g3)',borderRadius:6}}>
            <option value="abierto">Pendientes</option>
            <option value="resuelto">Resueltos</option>
            <option value="todos">Todos</option>
          </select>
          <span style={{fontSize:'0.63rem',color:'var(--g5)'}}>{list.length} reporte(s)</span>
        </div>
        <div className="mhist-wrap">
          {list.length===0
            ? <div className="hist-empty">Sin reportes {filter==='abierto'?'pendientes':filter==='resuelto'?'resueltos':''}.</div>
            : list.map((r,idx)=>(
              <div key={r._id||r.id||idx} className="hlog-item">
                <div className="hlog-dt">{r.fecha}<br/>{r.hora}</div>
                <div className={`hlog-op ${r.estado==='abierto'?'hop-del':'hop-add'}`}>
                  {r.estado==='abierto' ? <Icon.Alert/> : <Icon.Check/>} {r.estado==='abierto'?'PENDIENTE':'RESUELTO'}
                </div>
                <div className="hlog-det">
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10}}>
                    <div>
                      <div><strong>{r.tipo||'Reporte'}</strong> — Código: {r.codigo||'—'} ({r.marca}{r.modelo?` ${r.modelo}`:''})</div>
                      {r.comentario && <div>{r.comentario}</div>}
                      <div className="hlog-ip">{r.usuario||'anónimo'}{r.clasi?` · ${r.clasi}${r.sub?` / ${r.sub}`:''}`:''}</div>
                    </div>
                    <button className={`btn btn-sm ${r.estado==='abierto'?'btn-p':'btn-o'}`}
                      style={{flexShrink:0,whiteSpace:'nowrap'}}
                      disabled={busyId===r.id}
                      onClick={()=>toggle(r)}>
                      {busyId===r.id ? '…' : (r.estado==='abierto' ? 'Marcar resuelto' : 'Reabrir')}
                    </button>
                  </div>
                </div>
              </div>
...
// ── Modal de solo lectura: "Mis Reportes" (para viewer) ──
// Muestra únicamente lo que el propio usuario reportó y si ya fue
// atendido. Sin botones de acción — el viewer no puede modificar el
// estado, solo el admin (reportes_update_admin en Supabase respalda esto).
const ModalMisReportes = ({ reportes, onClose }) => {
  ...
  return (
    <div className="mo show">
      <div className="md" style={{maxWidth:860,width:'95vw'}}>
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Alert/>Mis Reportes</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="hist-toolbar">
          <span style={{display:'inline-flex',alignItems:'center',color:'var(--bm)'}}><Icon.Alert/></span>
          <select value={filter} onChange={e=>setFilter(e.target.value)} style={{border:'1.5px solid var(--g3)',borderRadius:6}}>
            <option value="todos">Todos</option>
            ...
            : list.map((r,idx)=>(
              <div key={r._id||r.id||idx} className="hlog-item">
                ...
                <div className="hlog-det">
                  <div><strong>{r.tipo||'Reporte'}</strong> — Código: {r.codigo||'—'} ({r.marca}{r.modelo?` ${r.modelo}`:''})</div>
                  {r.comentario && <div>{r.comentario}</div>}
                  {r.estado==='resuelto' && r.resuelto_at && (
                    <div className="hlog-ip">Atendido el {new Date(r.resuelto_at).toLocaleString('es-PA',{dateStyle:'short',timeStyle:'short'})}</div>
                  )}
                </div>
              </div>
            ))
...
function CatalogoApp() {
  const toast = useToast();
  const { isAdmin, user, role, signOut, setSeccion } = useAuth();
  ...
  const [reportesLoaded, setReportesLoaded] = useState(false); // evita mostrar la pastilla antes de tiempo
  const [showReportes, setShowReportes] = useState(false);
  const reportesAlertShown = useRef(false);
  // Vista de solo lectura para el viewer: sus propios reportes y si ya
  // fueron atendidos. RLS (reportes_select_propio_o_admin) ya filtra a solo
  // sus filas, así que reutilizamos fsGetReportes tal cual.
  const [misReportes, setMisReportes] = useState([]);
  const [showMisReportes, setShowMisReportes] = useState(false);
  const loadMisReportes = async () => {
    try {
      setMisReportes(await fsGetReportes());
    } catch(e) {
      toast('Error cargando tus reportes: ' + e.message, 'error');
    }
  };
  ...
  // Reportar qué está usando el usuario (búsqueda avanzada O filtros — lo
  // que tenga activo en este momento) para el panel de "usuarios conectados".
  // Antes solo se enviaba el texto de búsqueda; si el usuario solo estaba
  // usando los filtros de Marca/Modelo/etc. no se reportaba nada.
  useEffect(()=>{
    let termino = '';
    if (showDec && selectedCode) {
      termino = `Decodificando: ${selectedCode}`;
    } else if (debText.trim()) {
      termino = `Buscando: ${debText.trim()}`;
    } else {
      const filtrosActivos = [fMarca, fModelo, fPeriodo, fSub].filter(Boolean);
      if (filtrosActivos.length) termino = `Filtrando: ${filtrosActivos.join(' · ')}`;
    }
    setSeccion?.(termino || null);
  }, [showDec, debText, selectedCode, fMarca, fModelo, fPeriodo, fSub, setSeccion]);
  ...
  useEffect(()=>{
    (async()=>{
      try {
        ...
        if (omitidos>0) toast(`${omitidos.toLocaleString()} registros sin "Código Repuesto" fueron omitidos.`, 'info');
  ...
  // ── Novedades: última marca/modelo/código agregado o editado.
  //    Se muestra como una píldora flotante que se puede expandir a una
  //    ficha con el detalle, con paginación para recorrer varias novedades.
  //    Si no hay ninguna alta/edición reciente, cae a mostrar un dato de
  //    muestra de la base (el registro más reciente disponible) para que
  //    la píldora nunca quede vacía. ──
  const NOVEDADES_SEEN_KEY = 'acr_novedades_seen_at';
  const [novedades, setNovedades] = useState([]);
  const [novedadesExpanded, setNovedadesExpanded] = useState(false);
  const [novedadesIdx, setNovedadesIdx] = useState(0);
  const [novedadesSeenAt, setNovedadesSeenAt] = useState(()=>{
    try { return localStorage.getItem(NOVEDADES_SEEN_KEY) || ''; } catch { return ''; }
  });
  const [fallbackDismissed, setFallbackDismissed] = useState(false);
  useEffect(()=>{
    (async()=>{
      try {
        const rows = await fsGetRecentAdds(20);
        setNovedades(rows);
      } catch(e) {
        console.warn('[Novedades] no se pudieron cargar:', e.message);
        toast('No se pudieron cargar las novedades: ' + e.message, 'error');
      }
    })();
  },[]); // eslint-disable-line

  // Solo se muestran las novedades (AGREGAR/EDITAR) que el usuario aún no vio
  const novedadesPendientes = useMemo(()=>
    novedades.filter(n => !novedadesSeenAt || new Date(n.created_at||0) > new Date(novedadesSeenAt)),
    [novedades, novedadesSeenAt]);
  useEffect(()=>{ if (novedadesIdx >= novedadesPendientes.length) setNovedadesIdx(0); },[novedadesPendientes.length]); // eslint-disable-line

  const novedadActual = novedadesPendientes[novedadesIdx] || null;
  // Cruza la novedad con el registro completo actual para poder mostrar
  // Clasificación/Subclasificación, Especificación, Desc. Estándar y Modelo
  // Original en la ficha expandida.
  //   - AGREGAR: el código va en cambios[0].despues (se guarda siempre al crear).
  //   - EDITAR: el código solo aparece en cambios si ese campo cambió; si no,
  //     se intenta ubicar el registro por Marca+Modelo (mejor esfuerzo).
  const novedadRecord = useMemo(()=>{
    if (!novedadActual) return null;
    const cambiosArr = novedadActual.cambios || [];
    const codigo = cambiosArr.find(c=>c.campo==='Código Repuesto')?.despues || cambiosArr[0]?.despues || '';
    if (codigo) {
      const byCode = records.find(r=>r.fields[4]===codigo);
      if (byCode) return byCode;
    }
    const resumen = String(novedadActual.resumen||'').trim();
    const [marca, ...restoParts] = resumen.split(' ');
    const resto = restoParts.join(' ');
    return records.find(r=>
      cleanFilterVal(r.fields[0])===cleanFilterVal(marca) && resto.includes(cleanFilterVal(r.fields[1]))
    ) || null;
  },[novedadActual, records]);

  // Sin novedades pendientes: se muestra un dato de la base como referencia,
  // que además rota aleatoriamente cada 5s (ver temporizador más abajo).
  const [fallbackTick, setFallbackTick] = useState(0);
  const novedadFallback = useMemo(()=>{
    if (novedadActual || fallbackDismissed || !records.length) return null;
    const idx = Math.floor(Math.random()*records.length);
    return records[idx] || records[0] || null;
  },[novedadActual, fallbackDismissed, records, fallbackTick]); // eslint-disable-line

  // Objeto unificado que consume el render de la píldora/ficha, sin importar
  // si el origen es una novedad real (AGREGAR/EDITAR) o el dato de muestra.
  const novedadDisplay = useMemo(()=>{
    const rec = novedadActual ? novedadRecord : novedadFallback;
    const f = rec?.fields;
    if (!novedadActual && !novedadFallback) return null;
    return {
      kind: novedadActual ? (novedadActual.op==='EDITAR'?'editar':'agregar') : 'base',
      record: rec,
      vehiculo: f ? `${f[0]} ${f[1]}${f[3]?` · ${f[3]}`:''}` : (novedadActual?.resumen || ''),
      sub: (f && f[12]) || '',
      clasi: (f && f[11]) || '',
      codigo: (f && f[4]) || '',
      especificacion: (f && f[13]) || '',
      specLine: f ? [f[13], f[10]].filter(Boolean).join(' · ') : '',
      modeloOriginal: f && f[2],
    };
  },[novedadActual, novedadRecord, novedadFallback]);

  // Reproduce el "dindón" cada vez que la campanita suena/anima
  // (misma condición que dispara la animación novf-bell-ring visual).
  const dindonKey = novedadDisplay?.kind==='base' ? (novedadDisplay.codigo || novedadDisplay.vehiculo) : null;
  useEffect(()=>{ if (dindonKey) playDindon(); },[dindonKey]);

  const dismissNovedad = ()=>{
    if (novedadActual) {
      const now = new Date().toISOString();
      setNovedadesSeenAt(now);
      try { localStorage.setItem(NOVEDADES_SEEN_KEY, now); } catch {}
    } else {
      setFallbackDismissed(true);
    }
    setNovedadesExpanded(false);
  };

  // Click en la píldora / ficha: navega directo al dato — limpia otros
  // filtros y filtra por Marca+Código (o resumen, si no hay registro exacto)
  // para que aparezca de inmediato en la tabla.
  const goToNovedad = ()=>{
    if (!novedadDisplay) return;
    setFClasi(''); setFSub(''); setFLitraje(''); setFPeriodo('');
    setFModelo('');
    if (novedadDisplay.record) {
      setFMarca(cleanFilterVal(novedadDisplay.record.fields[0]));
      setFText(novedadDisplay.record.fields[4]); setDebText(novedadDisplay.record.fields[4]);
    } else {
      const marca = novedadDisplay.vehiculo.split(' ')[0] || '';
      setFMarca(cleanFilterVal(marca));
      setFText(novedadDisplay.vehiculo); setDebText(novedadDisplay.vehiculo);
    }
    setPage(1);
    setNovedadesExpanded(false);
  };
  const novedadesPrev = ()=> setNovedadesIdx(i=>Math.max(0, i-1));
  const novedadesNext = ()=> setNovedadesIdx(i=>Math.min(novedadesPendientes.length-1, i+1));

  // Bucle de la píldora: se muestra 10s, luego se oculta (fade) 2s y
  // durante ese hueco cambia al siguiente dato (novedad o, si no hay
  // pendientes, rota el dato de muestra de la base), reaparece y se
  // mantiene otros 10s. Si la ficha estaba expandida, se colapsa sola.
  const [novedadHidden, setNovedadHidden] = useState(false);
  useEffect(()=>{
    let showT, hideT;
    const cycle = () => {
      showT = setTimeout(()=>{
        setNovedadesExpanded(false);
        setNovedadHidden(true);
        hideT = setTimeout(()=>{
          if (novedadesPendientes.length > 1) {
            setNovedadesIdx(i => (i + 1) % novedadesPendientes.length);
          } else {
            setFallbackTick(t => t + 1);
          }
          setNovedadHidden(false);
          cycle();
        }, 2000);
      }, 10000);
    };
  ...
  // ── Reportes de usuarios ("ticket helpdesk") ──
  // Solo el admin puede verlos/gestionarlos. Se cargan apenas se detecta la
  // sesión admin (no lazy como el historial) para poder mostrar la alerta
  // de pendientes al ingresar y el badge junto al botón "Reportes".
  const loadReportes = async () => {
    try {
      const data = await fsGetReportes();
      setReportes(data);
  ...
  useEffect(() => {
    if (!isAdmin) return;
    (async () => {
      const data = await loadReportes();
      const pendientes = data.filter(r => r.estado === 'abierto').length;
      if (pendientes > 0 && !reportesAlertShown.current && !loading) {
        reportesAlertShown.current = true;
        toast(`⚠️ Tienes ${pendientes} reporte${pendientes===1?'':'s'} pendiente${pendientes===1?'':'s'} por revisar.`, 'warning', 7000);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAdmin, loading]);

  // El badge junto al botón "Reportes" antes solo se actualizaba al abrir
  // el modal (loadReportes se llamaba en el onClick). Si otro usuario
  // reporta mientras el admin ya tiene la app abierta, el número se veía
  // desactualizado hasta que el admin entraba manualmente. Este polling en
  // segundo plano (mismo patrón que el de "Usuarios") mantiene el contador
  // al día sin depender de que se abra el modal.
  useEffect(() => {
    if (!isAdmin) return;
    const iv = setInterval(() => { loadReportes(); }, 15000);
    return () => clearInterval(iv);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAdmin]);

  // Agrega el reporte recién enviado a la lista local de inmediato, para que
  // aparezca en el historial sin esperar a un refetch (que además puede
  // fallar por permisos si quien reporta no es admin).
  const addReporteLocal = (entry) => {
    setReportes(prev => [entry, ...prev]);
    // También lo reflejamos en el Historial de Cambios, para que quede
    // registrado que se reportó un código (antes solo quedaba en el panel
    // de Reportes, y no se veía en el historial general).
    const desc = `${entry.marca || ''} ${entry.modelo || ''}`.trim() || (entry.codigo || 'Registro');
    logEntry('REPORTAR', `${desc} — ${entry.tipo || 'Reporte'}`,
      [{campo:'Comentario', antes:'', despues:entry.comentario || ''}]);
  };

  // Refleja localmente el resultado de una verificación/revocación sin
  // esperar a un refetch completo de la tabla.
  const handleVerified = (id, verificado) => {
    setRecords(prev => prev.map(r => r._id===id ? {...r, verificado} : r));
  };

  const updateReporteEstado = async (id, estado) => {
    try {
      await fsUpdateReporteEstado(id, estado, estado==='resuelto' ? user?.email : null);
      setReportes(prev => prev.map(r => r.id===id ? {...r, estado} : r));
      toast(estado==='resuelto' ? 'Reporte marcado como resuelto.' : 'Reporte reabierto.', 'success');
    } catch(e) {
      toast('Error al actualizar el reporte: ' + e.message, 'error');
    }
  };
  ...
  // ── Sesiones de usuario (ingreso/salida + quién está conectado ahora) ──
  const [sessions, setSessions] = useState([]);
  const [showSessions, setShowSessions] = useState(false);
  const loadSessions = async () => {
    try {
      setSessions(await fsGetSessions());
    } catch(e) {
      toast('Error cargando sesiones: ' + e.message, 'error');
    }
  };
  // Mientras el modal de Usuarios está abierto, refrescamos periódicamente
  // para que "qué está buscando" y quién sigue en línea se vean casi en
  // vivo, sin que el admin tenga que cerrar y volver a abrir el modal.
  useEffect(() => {
  ...
  // Clasificación/Subclasificación son filtros independientes de
  // Marca/Modelo/Período: cambiarlas no toca la selección de vehículo.
  ...
  const logEntry = async (op, resumen, cambios=[])=>{
    const {fecha,hora}=nowDT();
    const entry={op,resumen,cambios,fecha,hora,id:Date.now(),usuario:user?.email||'desconocido'};
  ...
  // ── Importar ──
  const handleImport = async (rows, mode, target=dataSource, meta={})=>{
    const { motivo='', fileName='' } = meta;
    ...
    try{
      ...
      if (targetTable === currentTable) {
        ...
        toast(`${total} registros importados a "${DATA_SOURCES[target].label}".${omitidosImport?` ${omitidosImport.toLocaleString()} quedaron ocultos por no tener "Código Repuesto".`:''}`,'success');
      } else {
      ...
      await logEntry('IMPORTAR',
        `${mode==='replace'?'Reemplazo':'Adición'} de ${total} registros en ${DATA_SOURCES[target].label}`
  ...
  return (
    ...
    <>
      ...
      <div className="ac-header">
        <div className="ac-hl">
          ...
          <div className="ac-htitle">
            ...
            <span className="s2">Aplicación de Repuestos</span>
          </div>
          ...
          {isAdmin ? (
            <div style={{position:'relative',display:'inline-block'}}>
              <button
                ...
                title={isPreviewing
                  ? 'Cambio sin publicar: los demás usuarios no ven esto todavía'
                    ...
                    {isPreviewing ? (
                      <>
                        <div style={{
                          ...
                          Estás <strong>solo previsualizando</strong> "{DATA_SOURCES[dataSource].label}".
                        ...
                        <button
                          ...
                          2. Definir "{DATA_SOURCES[dataSource].label}" como fuente activa para TODOS
                        </button>
                      </>
                    ) : (
        ...
        <div className="ac-hact">
          {isAdmin && <button className="btn btn-p"
            onClick={()=>setModalEdit({_id:null,fields:Array(14).fill('')})}><Icon.Plus style={{marginRight:6,verticalAlign:-2}}/>Nuevo</button>}
          {isAdmin && <button className="btn btn-c" onClick={()=>{ loadReportes(); setShowReportes(true); }}>
            <Icon.Alert style={{marginRight:6,verticalAlign:-2}}/>Reportes
            {reportesLoaded && reportes.filter(r=>r.estado==='abierto').length>0 && <span style={{background:'#C62828',color:'#fff',
              borderRadius:10,padding:'1px 7px',fontSize:'0.57rem',marginLeft:4}}>
              {reportes.filter(r=>r.estado==='abierto').length}</span>}
          </button>}
          {!isAdmin && <button className="btn btn-c" onClick={()=>{ loadMisReportes(); setShowMisReportes(true); }}>
            <Icon.Alert style={{marginRight:6,verticalAlign:-2}}/>Mis Reportes
          </button>}
          {isAdmin && <div style={{position:'relative',display:'inline-block'}}>
            <button className="btn btn-c" onClick={()=>setShowBaseMenu(!showBaseMenu)}><Icon.Folder style={{marginRight:6,verticalAlign:-2}}/>Cargar base</button>
            {showBaseMenu && (
              <div style={{position:'absolute',top:'100%',left:0,marginTop:4,background:'#fff',border:'1px solid var(--g3)',
                ...
                <button onClick={()=>{setShowImport(true);setShowBaseMenu(false);}} 
                  style={{width:'100%',padding:'10px 14px',border:'none',background:'none',textAlign:'left',cursor:'pointer',fontSize:'0.72rem',borderBottom:'1px solid var(--g2)',transition:'.15s',display:'flex',alignItems:'center',gap:8}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--bl)'}
                  onMouseLeave={e=>e.currentTarget.style.background='none'}>
                  <Icon.Upload style={{flexShrink:0,color:'var(--bm)'}}/>Carga de la base
                </button>
                <button onClick={()=>{exportCSV();setShowBaseMenu(false);}}
                  style={{width:'100%',padding:'10px 14px',border:'none',background:'none',textAlign:'left',cursor:'pointer',fontSize:'0.72rem',borderBottom:'1px solid var(--g2)',transition:'.15s',display:'flex',alignItems:'center',gap:8}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--bl)'}
                  onMouseLeave={e=>e.currentTarget.style.background='none'}>
                  <Icon.Download style={{flexShrink:0,color:'var(--bm)'}}/>Descarga de la base
                </button>
                <button onClick={()=>{decActionsRef.current?.triggerUpload();setShowBaseMenu(false);}}
                  style={{width:'100%',padding:'10px 14px',border:'none',background:'none',textAlign:'left',cursor:'pointer',fontSize:'0.72rem',borderBottom:'1px solid var(--g2)',transition:'.15s',display:'flex',alignItems:'center',gap:8}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--bl)'}
                  onMouseLeave={e=>e.currentTarget.style.background='none'}>
                  <Icon.Upload style={{flexShrink:0,color:'var(--bm)'}}/>Carga de decodificador
                </button>
                <button onClick={()=>{decActionsRef.current?.handleDownloadBD();setShowBaseMenu(false);}}
                  style={{width:'100%',padding:'10px 14px',border:'none',background:'none',textAlign:'left',cursor:'pointer',fontSize:'0.72rem',transition:'.15s',display:'flex',alignItems:'center',gap:8}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--bl)'}
                  onMouseLeave={e=>e.currentTarget.style.background='none'}>
                  <Icon.Download style={{flexShrink:0,color:'var(--bm)'}}/>Descarga de decodificador
                </button>
              </div>
            )}
          </div>}
          <button className="btn btn-c" onClick={()=>setShowCols(true)}><Icon.Eye style={{marginRight:6,verticalAlign:-2}}/>Columnas</button>
          {isAdmin && <button className="btn btn-c" onClick={()=>setShowReplace(true)}><Icon.Refresh style={{marginRight:6,verticalAlign:-2}}/>Reemplazar</button>}
          {isAdmin && <button className="btn btn-c" onClick={()=>{ loadChangelog(); setShowHistory(true); }}>
            <Icon.History style={{marginRight:6,verticalAlign:-2}}/>Historial
            {changelog.length>0&&<span style={{background:'var(--gold)',color:'var(--bd)',
              borderRadius:10,padding:'1px 7px',fontSize:'0.57rem',marginLeft:4}}>
              {changelog.length}</span>}
          </button>}
          {isAdmin && <button className="btn btn-c" onClick={()=>{ loadSessions(); setShowSessions(true); }}>
            <Icon.Eye style={{marginRight:6,verticalAlign:-2}}/>Usuarios
          </button>}
          <div style={{display:'flex',alignItems:'center',gap:6,marginLeft:4,paddingLeft:8,borderLeft:'1px solid rgba(255,255,255,.2)'}}>
            ...
            <button className="btn btn-c btn-sm" onClick={signOut} title="Cerrar sesión"
              style={{padding:'3px 9px',fontSize:'0.55rem'}}><Icon.LogOut style={{marginRight:4,verticalAlign:-2}}/>Salir</button>
          </div>
        </div>
      </div>

      {/* VENTANA DE CARGA — modal emergente centrado, estilo popup moderno.
          Spinner grande de doble color, sin barra de progreso ni video. */}
      <div className={`ac-progress-overlay${loadProgress.active?' active':''}`}>
        <div className="ac-progress-card">
          <div className="ac-progress-spin-wrap">
            <div className="ac-progress-spin"/>
          </div>
          <div className="ac-progress-title">Cargando aplicación</div>
      ...
      {/* FILTROS */}
      <div className={`ac-sp${(findByNormalizedKey(SUBCLASI_IMAGES,fSub) || findByNormalizedKey(CLASI_IMAGES,fClasi) || (!fSub && !fClasi)) ? ' has-clasi-img' : ''}`}>
        {(() => {
          const renderFilter = ({label,icon,val,set,opts,placeholder='Todas las marcas'}) => {
          ...
          return (<>
            {/* LAYOUT 1: Marca / Modelo / Período / Litraje — siempre visibles,
                sin depender de que haya Clasificación/Subclasificación elegida. */}
            <div className="ac-fg">
              {renderFilter({label:'Marca', icon:<Icon.Bookmark style={{width:11,height:11}}/>, val:fMarca, set:onMarcaChange, opts:availableMarcas})}
              {renderFilter({label:'Modelo', icon:<Icon.Car style={{width:11,height:11}}/>, val:fModelo, set:onModeloChange, opts:availableModels, placeholder:'Todos los modelos'})}
              {renderFilter({label:'Período', icon:<Icon.Calendar style={{width:11,height:11}}/>, val:fPeriodo, set:onPeriodoChange, opts:availablePeriodos, placeholder:'Todos los períodos'})}
              {renderFilter({label:'Especificación', icon:<Icon.Ruler style={{width:11,height:11}}/>, val:fLitraje, set:onLitrajeChange, opts:availableLitrajes, placeholder:'Todas'})}
            </div>

            {/* LAYOUT 2: Clasificación / Subclasificación — selección secundaria.
                La píldora de Novedades ocupa las columnas libres de esta misma
                fila (el grid tiene 4 columnas y solo se usan 2), alineada a la
                derecha y a la misma altura que los selects. */}
            <div className="ac-fg2 ac-fg2-primary">
              {renderFilter({label:'Clasificación', icon:<Icon.Layers style={{width:11,height:11}}/>, val:fClasi, set:onClasiChange, opts:availableClasif, placeholder:'Todas'})}
              ...
              {novedadDisplay && (()=>{
                ...
                return (
                  <div className="novf-inline-wrap" key={codLine||vehiculo} style={{gridColumn:'3 / span 2'}}>
                    <div className="novf-pill-holder">
                      <button type="button" className="novf-close-float" onClick={e=>{e.stopPropagation();dismissNovedad();}} title="Descartar">
                        <Icon.X style={{width:10,height:10}}/>
                      </button>
                      <div className={`novf-pill novf-pill-rise${novedadHidden?' novf-hide':''}`} onClick={goToNovedad} title="Ver este registro en la tabla">
                        {kind==='base' ? <Icon.Bell key={codLine||vehiculo} className="novf-bell-ring" style={{width:13,height:13,flexShrink:0}}/> : <Icon.Alert style={{width:13,height:13,flexShrink:0}}/>}
                        <span className="novf-veh">{vehiculo}</span>
                        <span className="novf-sep"/>
                        <span className="novf-cat-icon"><ClasiIcon clasi={clasi} sub={sub} size={18} forceSvg/></span>
                        <span className="novf-cat">
                          <b>{(sub||clasi||'Novedad').toUpperCase()}</b>
                          {codLine && <small>{codLine}</small>}
                        </span>
                        <span className="novf-chev-btn" onClick={e=>{e.stopPropagation();setNovedadesExpanded(v=>!v);}} title="Ver detalle">
                          <span className={`novf-chev${novedadesExpanded?' open':''}`}>›</span>
                        </span>
                      </div>
                    </div>
                    {novedadesExpanded && (
                      <div className="novf-dropdown">
                        <div className="novf-card" onClick={goToNovedad} title="Ver este registro en la tabla">
                          <div className="novf-tagline">{kindLabel}</div>
                          <div className="novf-row">
                            <span className="novf-ic"><Icon.Car style={{width:12,height:12}}/></span>
                            <div><b>{vehiculo}</b></div>
                          </div>
                          <div className="novf-row">
                            <span className="novf-ic"><ClasiIcon clasi={clasi} sub={sub} size={13} forceSvg/></span>
                            <div>
                              <b className="novf-gold">{(sub||clasi||'—').toUpperCase()}</b>
                              {specLine && <div className="novf-small">{specLine}</div>}
                            </div>
                          </div>
                          <div className="novf-row">
                            <span className="novf-ic"><Icon.Tag style={{width:12,height:12}}/></span>
                            <div>Código: <b>{codigo||'—'}</b></div>
                          </div>
                          {modeloOriginal && (
                            <div className="novf-row novf-row-last">
                              <span className="novf-ic">★</span>
                              <div><b>{modeloOriginal}</b></div>
                            </div>
                          )}
                        </div>
                        {novedadesPendientes.length>1 && (
                          <div className="novf-pager" onClick={e=>e.stopPropagation()}>
                            <button type="button" disabled={novedadesIdx===0} onClick={novedadesPrev}>‹</button>
                            <span>{novedadesIdx+1} - {novedadesIdx+1} de {novedadesPendientes.length}</span>
                            <button type="button" disabled={novedadesIdx>=novedadesPendientes.length-1} onClick={novedadesNext}>›</button>
                          </div>
                        )}
                      </div>
                    )}
            ...
            {/* Ícono decorativo de fondo — flota a la derecha, cubriendo TODA la
                ...
                Detrás de los filtros (z-index:0 en el panel), pero como <img>
                real posicionado de forma absoluta para poder controlar su
                tamaño libremente sin que el contenedor la recorte. */}
            <div className="ac-clasi-icon-panel" title={fSub || fClasi || 'Todas las categorías'}>
              <ClasiIcon clasi={fClasi} sub={fSub} />
            </div>
          </>);
        })()}
      </div>
      ...
      <div className="ac-sb">
        ...
        <span style={{marginLeft:'auto',display:'flex',gap:5,flexWrap:'nowrap'}}>
          ...
          {fModelo&&<span className="ac-tag" style={{background:'rgba(255,255,255,.25)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Car style={{width:11,height:11}}/>{fModelo}</span>}
          {fPeriodo  &&<span className="ac-tag" style={{background:'rgba(212,168,0,.8)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Calendar style={{width:11,height:11}}/>{fPeriodo}</span>}
          {fClasi &&<span className="ac-tag" style={{background:'rgba(255,255,255,.25)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Layers style={{width:11,height:11}}/>{fClasi.substring(0,22)}</span>}
          {fSub   &&<span className="ac-tag" style={{background:'rgba(255,255,255,.25)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Tag style={{width:11,height:11}}/>{fSub}</span>}
          {fLitraje &&<span className="ac-tag" style={{background:'rgba(255,255,255,.25)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Ruler style={{width:11,height:11}}/>{fLitraje}</span>}
          {debText&&<span className="ac-tag" style={{background:'rgba(212,168,0,.8)',display:'inline-flex',alignItems:'center',gap:4}}><Icon.Search style={{width:11,height:11}}/>"{debText}"</span>}
        </span>
      </div>

      {/* BÚSQUEDA LIBRE — entre resultados y la tabla. La búsqueda ya filtra en vivo
          al escribir (onTextInput), así que se quitó el botón "Buscar" por redundante. */}
      <div className="ac-sr-mid">
        <button className="btn btn-clear" onClick={clearAll}><Icon.X style={{marginRight:6,verticalAlign:-2}}/>Limpiar</button>
        <div className="ac-fl">
      ...
      <div className="ac-tw">
        {loading ? (
          /* Spinner de la tabla ocultado — la ventana de carga global
             (ac-progress-*) ya cubre el estado de carga inicial. */
          null
        ) : records.length===0 ? (
          <div className="empty">
            <div className="icon"><Icon.Folder style={{width:40,height:40}}/></div>
          ...
          <div className="empty"><div className="icon"><Icon.Search style={{width:40,height:40}}/></div><p>No se encontraron resultados.</p></div>
        ) : (
          <table>
            <thead><tr>
              <th style={{width:80,minWidth:70,background:'linear-gradient(135deg,var(--bd) 0%,#15335c 100%)'}}>Acciones</th>
            ...
            <tbody>
              {paginated.map((rec,ri)=>{
                ...
                const cell={
                  ...
                  4:()=>f[4]?<span style={{display:'inline-flex',alignItems:'center',gap:6}}>
                    <span className="cr-pill">{highlightText(f[4],debText)}
                      <button className="btn-copy" onClick={e=>{e.stopPropagation();navigator.clipboard?.writeText(f[4]);toast('Código copiado','info');}}><Icon.Copy/></button>
                    </span>
                    {rec.verificado && <MiniVerifBadge/>}
                  ...
                  12:()=><span className="cm">{f[12]}{f[10]&&<><br/><span className="cmo">{f[10]}</span></>}</span>,
                ...
                return (
                  <tr key={rec._id||ri}>
                    <td className="cac" onClick={e=>e.stopPropagation()} style={{width:80,minWidth:70}}>
                      {isAdmin ? <>
                        <button className="btn-edit" onClick={()=>setModalEdit(rec)}><Icon.Pencil style={{verticalAlign:-2}}/></button>
                        <button className="btn-del"  onClick={()=>setModalDel(rec)}><Icon.Trash style={{verticalAlign:-2}}/></button>
                      </> : <button className="btn-edit" onClick={()=>setModalDetail(rec)}><Icon.Eye style={{verticalAlign:-2}}/></button>}
                    </td>
                    {activeCols.map(col=>(
      ...
      {modalDetail && <ModalDetail  record={modalDetail}  onClose={()=>setModalDetail(null)} onEdit={r=>{setModalDetail(null);setModalEdit(r);}} onReported={addReporteLocal} onVerified={handleVerified}/>}
      {showImport  && <ModalImport  onClose={()=>setShowImport(false)}  onImport={handleImport} defaultTarget={dataSource}/>}
      {showCols    && <ModalCols    visibleCols={visibleCols} colOrder={colOrder} onChange={(i,s)=>setVisibleCols(v=>v.map((c,ci)=>ci===i?{...c,show:s}:c))} onReorder={setColOrder} onRestore={()=>{setColOrder([...COL_DEFS_ORDER]);setVisibleCols(COL_DEFS.map(c=>({...c})));}} onClose={()=>setShowCols(false)}/>}
      {showHistory && <ModalHistory changelog={changelog} onClose={()=>setShowHistory(false)}/>}
      {showSessions && <ModalSessions sessions={sessions} onClose={()=>setShowSessions(false)}/>}
      {showReportes && <ModalReportes reportes={reportes} onClose={()=>setShowReportes(false)} onUpdateEstado={updateReporteEstado}/>}
      {showMisReportes && <ModalMisReportes reportes={misReportes} onClose={()=>setShowMisReportes(false)}/>}
