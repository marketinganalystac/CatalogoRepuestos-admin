...
.dtl-md{max-width:920px;width:96vw}
.dtl-hleft{display:flex;align-items:center;gap:12px;min-width:0}
.dtl-hicon{width:42px;height:42px;border-radius:50%;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);\n  display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff}
.dtl-htitles{display:flex;flex-direction:column;gap:3px;min-width:0}
.dtl-htitles h2{font-size:0.86rem;color:#fff;font-weight:700;line-height:1.1}
.dtl-hrow2{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.dtl-hcode{font-size:0.76rem;color:#BFDCF5;font-weight:700;font-family:'SF Mono','Fira Code','Consolas',monospace;\n  overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:300px}
.dtl-verif-pill{display:inline-flex;align-items:center;gap:6px;background:#0071c2;color:#fff;border-radius:20px;\n  padding:4px 12px 4px 9px;font-size:0.6rem;font-weight:700;white-space:nowrap;flex-shrink:0;box-shadow:0 1px 4px rgba(0,0,0,.15)}
.dtl-body{padding:18px 22px 4px;display:flex;flex-direction:column;gap:14px}
.dtl-grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:stretch}
.dtl-col{display:flex;flex-direction:column;gap:8px}
.dtl-item{display:flex;align-items:flex-start;gap:11px;background:var(--g1);border:1px solid var(--g2);border-radius:9px;padding:9px 13px}
.dtl-item-ic{width:32px;height:32px;border-radius:9px;background:var(--bl);color:var(--bm);display:flex;\n  align-items:center;justify-content:center;flex-shrink:0}
.dtl-item-body{min-width:0;flex:1}
.dtl-item-lbl{font-size:0.53rem;font-weight:700;color:var(--bm);text-transform:uppercase;letter-spacing:.6px;margin-bottom:2px}
.dtl-item-val{font-size:0.78rem;font-weight:700;color:var(--g9);word-break:break-word;line-height:1.3}
.dtl-item-val.empty{color:var(--g5);font-weight:500;font-style:italic}
.dtl-codes-card{background:var(--g1);border:1px solid var(--g2);border-radius:9px;padding:11px 12px;display:flex;\n  flex-direction:column;gap:7px;height:100%}
.dtl-codes-title{display:flex;align-items:center;gap:9px;font-size:0.6rem;font-weight:700;color:var(--bm);\n  text-transform:uppercase;letter-spacing:.6px;margin-bottom:2px}
.dtl-codes-ic{width:26px;height:26px;border-radius:8px;background:var(--bl);color:var(--bm);display:flex;\n  align-items:center;justify-content:center;flex-shrink:0}
.dtl-code-row{display:flex;align-items:center;gap:10px;background:#fff;border:1px solid var(--g2);border-radius:7px;padding:6px 10px}
.dtl-code-num{width:20px;height:20px;border-radius:50%;background:var(--bl);color:var(--bm);font-size:0.6rem;\n  font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.dtl-code-val{flex:1;font-family:'SF Mono','Fira Code','Consolas',monospace;font-weight:700;color:var(--g9);font-size:0.73rem;\n  word-break:break-all}
.dtl-code-copy{background:none;border:none;color:var(--bm);cursor:pointer;display:flex;align-items:center;\n  justify-content:center;padding:4px;border-radius:5px;flex-shrink:0}
.dtl-code-copy:hover{background:var(--bl)}
...
const ModalDetail = ({ record, onClose, onEdit, onReported, onVerified }) => {
  ...
  // Orden observado en la base: código_1, código_2, código_repuesto (combinado),
  // código_3, código_4, código_5 — se numeran del 1 en adelante solo los que existen.
  const codigosLista = [5,6,4,7,8,9].map(i=>f[i]).filter(v=>v && v.trim());
    ...
    <div className="mo show">
      <div className="md dtl-md">
            ...
            <div className="dtl-htitles">
              ...
              <div className="dtl-hrow2">
                ...
                <span className="dtl-verif-pill" style={estaVerificado?undefined:{background:'#546E7A',cursor:(!verifsLoaded||verifSaving||yaVerifique)?'default':'pointer'}}
                  onClick={!estaVerificado?confirmarVerificacion:undefined}
                  title={estaVerificado?'Verificado por la comunidad':(yaVerifique?'Ya confirmaste — falta al menos 1 confirmación más':`Confirmar código (${new Set(verifs.map(v=>v.usuario)).size}/${VERIF_THRESHOLD})`)}>
                  {estaVerificado ? <Icon.Check style={{width:12,height:12}}/> : <Icon.Alert style={{width:11,height:11}}/>}
                  {estaVerificado ? 'Verificado' : `Sin verificar (${new Set(verifs.map(v=>v.usuario)).size}/${VERIF_THRESHOLD})`}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="dtl-body">
          ...
          {/* Campos adicionales — ancho completo */}
          <div className="dtl-item">
            <div className="dtl-item-ic"><Icon.Doc/></div>
            <div className="dtl-item-body">
              <div className="dtl-item-lbl">Descripción</div>
              <div className={`dtl-item-val${f[10]?'':' empty'}`}>{f[10] || '—'}</div>
            </div>
          </div>
          <div className="dtl-item">
            <div className="dtl-item-ic"><Icon.Doc/></div>
            <div className="dtl-item-body">
              <div className="dtl-item-lbl">Modelo Original</div>
              <div className={`dtl-item-val${f[2]?'':' empty'}`}>{f[2] || '—'}</div>
            </div>
          </div>
          <div className="dtl-item">
            <div className="dtl-item-ic"><Icon.Link/></div>
            <div className="dtl-item-body">
              <div className="dtl-item-lbl">Equivalencia</div>
              <div className="dtl-item-val empty">—</div>
            </div>
          </div>
