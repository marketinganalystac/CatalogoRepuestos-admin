...
const ModalDetail = ({ record, onClose, onEdit, onReported, onVerified }) => {
  ...
  const faltan = Math.max(0, VERIF_THRESHOLD - verifs.length);
  const efectivamenteVerificado = record.verificado || new Set(verifs.map(v=>v.usuario)).size >= VERIF_THRESHOLD;
  ...
  return (
    <div className="mo show">
      <div className="md sm">
        <div className="mh"><h2 style={{display:'flex',alignItems:'center',gap:8}}><Icon.Search/>Detalle del Registro</h2><button className="mx" onClick={onClose}>×</button></div>
        <div className="mb">
          ...
          <div style={{marginTop:14,padding:12,borderRadius:8,background: efectivamenteVerificado ? '#E8F2FA' : 'var(--g1)',border:`1px solid ${efectivamenteVerificado?'#90CAF9':'var(--g3)'}`}}>
            <div style={{fontSize:'0.68rem',fontWeight:700,color:'#0060A0',marginBottom:6,display:'flex',alignItems:'center',gap:6}}>
              {efectivamenteVerificado ? <>✔✔ Verificado por la comunidad</> : <>Verificación de exactitud</>}
            </div>
            {!efectivamenteVerificado && (
              <div style={{fontSize:'0.65rem',color:'var(--g6)',marginBottom:8}}>
                {verifsLoaded
                  ? (verifs.length===0
                      ? `Confirma si este código coincide con lo real. Se necesitan ${VERIF_THRESHOLD} confirmaciones de usuarios distintos.`
                      : `${verifs.length} de ${VERIF_THRESHOLD} confirmaciones. Falta${faltan===1?'':'n'} ${faltan}.`)
                  : 'Cargando…'}
              </div>
            )}
            {!efectivamenteVerificado && (
              <button className="btn btn-sm" style={{background:'#0060A0',color:'#fff'}}
                disabled={!verifsLoaded || yaVerifique || verifSaving}
                onClick={confirmarVerificacion}>
                {verifSaving ? 'Confirmando…' : yaVerifique ? 'Ya confirmaste' : 'Confirmar que es correcto'}
              </button>
            )}
          </div>
